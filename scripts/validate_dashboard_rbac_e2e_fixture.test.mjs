...
test('fixture validator requires the complete account matrix and exact route contracts', () => {
  const fixture = Object.fromEntries(['tenantDenied', 'noPermission', 'direct', 'twoRole', 'roleDisabledDirectRetained', 'ordinary48', 'superadmin'].map((name) => [name, account([])]));
  fixture.managedUserId = 5;
  ...
  fixture.ordinary48.exactAllowedRoutes = [...routes.slice(0, 47), '/bad'];
...
test('fixture validator rejects missing exact routes', () => {
  assert.throws(() => validateFixture({ managedUserId: 5 }, { pages: [] }), /tenantDenied requires/);
});

test('fixture validator requires the managed ordinary user identifier', () => {
  assert.throws(() => validateFixture({}, { pages: [] }), /managedUserId/);
});
