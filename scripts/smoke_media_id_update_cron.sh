...
PROJECT_NAME="${MOCHAT_STACK_PROJECT:-mochat-go-media-id-cron-check}"
GO_ADDR="${MOCHAT_GO_ADDR:-127.0.0.1:18089}"
MYSQL_PORT="${MOCHAT_MYSQL_PORT:-13321}"
REDIS_PORT="${MOCHAT_REDIS_PORT:-26470}"
WECOM_ADDR="${MOCHAT_WECOM_ADDR:-127.0.0.1:19055}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mochat-go-media-id-cron.XXXXXX")"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path.path == "/cgi-bin/media/upload":
            ...
            media_type = parse_qs(path.query).get("type", ["file"])[0]
            self.json_response({"errcode": 0, "errmsg": "ok", "media_id": f"cron-{media_type}-media-id"})
            return
        self.send_response(404)
        self.end_headers()
...
INSERT INTO mc_corp (
  ...
  1, 'Go素材企业', 'ww-media', 'employee-secret', 'contact-secret',
...
env -u GOROOT \
  ...
  MOCHAT_GO_ENABLE_MEDIA_ID_UPDATE_CRON=1 \
  MOCHAT_GO_MEDIA_ID_UPDATE_CRON_RUN_ON_START=1 \
  MOCHAT_GO_MEDIA_ID_UPDATE_CRON_INTERVAL_SECONDS=60 \
  "$GO_BIN" >"$GO_LOG" 2>&1 &
GO_PID="$!"
...
task = tasks.get("cron-media-id-update")
assert task, status
assert task.get("status") == "running", task
assert task.get("started_at"), task
PY

wait_mysql_scalar "SELECT media_id FROM mc_medium WHERE id = 1" "cron-image-media-id"
wait_mysql_scalar "SELECT IF(last_upload_time > 1, 'yes', 'no') FROM mc_medium WHERE id = 1" "yes"

grep -q "go cron enabled: mediaIdUpdate" "$GO_LOG"
grep -q "mediaIdUpdate cron finished: corps=1 scanned=1 updated=1 skipped=0" "$GO_LOG"

echo "mediaIdUpdate cron smoke passed"
