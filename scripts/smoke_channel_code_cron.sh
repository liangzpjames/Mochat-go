...
PROJECT_NAME="${MOCHAT_STACK_PROJECT:-mochat-go-channel-code-cron-check}"
GO_ADDR="${MOCHAT_GO_ADDR:-127.0.0.1:18093}"
MYSQL_PORT="${MOCHAT_MYSQL_PORT:-13325}"
REDIS_PORT="${MOCHAT_REDIS_PORT:-26470}"
WECOM_ADDR="${MOCHAT_WECOM_ADDR:-127.0.0.1:19059}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mochat-go-channel-code-cron.XXXXXX")"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.path == "/cgi-bin/gettoken":
            assert query.get("corpid", [""])[0] == "ww-channel", query
    ...
    def do_POST(self):
        ...
        if path.path == "/cgi-bin/externalcontact/add_contact_way":
            ...
            assert payload.get("user") == ["go-user"], payload
            if payload.get("state") == "channelCode-2":
                self.json_response({"errcode": 40001, "errmsg": "expected create failure"})
                return
            assert payload.get("state") == "channelCode-1", payload
            self.json_response({
                "errcode": 0,
                "errmsg": "ok",
                "qr_code": "https://example.com/channel-code.png",
                "config_id": "config-channel-new"
            })
            return
        self.send_response(404)
        self.end_headers()
...
DELETE FROM mc_channel_code WHERE id IN (1, 2);
...
INSERT INTO mc_corp (
  ...
  1, 'Go渠道码企业', 'ww-channel', 'employee-secret', 'contact-secret',
...
INSERT INTO mc_channel_code (
  id, corp_id, group_id, name, qrcode_url, wx_config_id, auto_add_friend, tags, type,
  drainage_employee, welcome_message, created_at, updated_at, deleted_at
) VALUES (
  1, 1, 0, 'Go渠道码', '', '', 1, JSON_ARRAY(), 1,
  JSON_OBJECT(
    'employees', JSON_ARRAY(JSON_OBJECT('week', $WEEK, 'timeSlot', JSON_ARRAY(JSON_OBJECT('startTime', '00:00', 'endTime', '00:00', 'employeeId', JSON_ARRAY(1))))),
    'specialPeriod', JSON_OBJECT('status', 2, 'detail', JSON_ARRAY()),
    'addMax', JSON_OBJECT('status', 2, 'employees', JSON_ARRAY(), 'spareEmployeeIds', JSON_ARRAY())
  ),
  JSON_OBJECT(), '$TODAY 08:20:00', '$TODAY 08:20:00', NULL
);
...
INSERT INTO mc_channel_code (
  ...
  2, 1, 0, 'Go渠道码失败回滚', '', '', 1, JSON_ARRAY(), 1,
  JSON_OBJECT(
    'employees', JSON_ARRAY(JSON_OBJECT('week', $WEEK, 'timeSlot', JSON_ARRAY(JSON_OBJECT('startTime', '00:00', 'endTime', '00:00', 'employeeId', JSON_ARRAY(1))))),
    'specialPeriod', JSON_OBJECT('status', 2, 'detail', JSON_ARRAY()),
    'addMax', JSON_OBJECT('status', 2, 'employees', JSON_ARRAY(), 'spareEmployeeIds', JSON_ARRAY())
  ),
  JSON_OBJECT(), '$TODAY 08:25:00', '$TODAY 08:25:00', NULL
);

DELETE FROM mochat_go_saas_usage_counters
WHERE tenant_id = 1 AND metric = 'channel_codes';
...
VALUES
  (1, 'channel_codes', 'lifetime', 2, 20, 'seed', NOW(), NOW(), NULL);
SQL
...
env -u GOROOT \
  ...
  MOCHAT_GO_ENABLE_CHANNEL_CODE_CRON=1 \
  MOCHAT_GO_CHANNEL_CODE_CRON_RUN_ON_START=1 \
  MOCHAT_GO_CHANNEL_CODE_CRON_INTERVAL_SECONDS=60 \
  "$GO_BIN" >"$GO_LOG" 2>&1 &
GO_PID="$!"
...
task = tasks.get("cron-channel-code")
assert task, status
assert task.get("status") == "running", task
assert task.get("started_at"), task
PY

wait_mysql_scalar "SELECT CONCAT(qrcode_url, '|', wx_config_id) FROM mc_channel_code WHERE id = 1" "https://example.com/channel-code.png|config-channel-new"
wait_mysql_scalar "SELECT COUNT(*) FROM mc_channel_code WHERE id = 2 AND deleted_at IS NOT NULL" "1"
wait_mysql_scalar "SELECT used_value FROM mochat_go_saas_usage_counters WHERE tenant_id = 1 AND metric = 'channel_codes' AND deleted_at IS NULL" "1"

grep -q "go cron enabled: channelCode" "$GO_LOG"
grep -q "channelCode cron finished: scanned=2 updated=1 skipped=0 failed=1" "$GO_LOG"

echo "channelCode cron smoke passed"
