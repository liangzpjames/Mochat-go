...
PROJECT_NAME="${MOCHAT_STACK_PROJECT:-mochat-go-contact-batch-send-cron-check}"
GO_ADDR="${MOCHAT_GO_ADDR:-127.0.0.1:18097}"
MYSQL_PORT="${MOCHAT_MYSQL_PORT:-13329}"
REDIS_PORT="${MOCHAT_REDIS_PORT:-26470}"
WECOM_ADDR="${MOCHAT_WECOM_ADDR:-127.0.0.1:19063}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mochat-go-contact-batch-send-cron.XXXXXX")"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.path == "/cgi-bin/gettoken":
            assert query.get("corpid", [""])[0] == "ww-scheduled", query
    ...
    def do_POST(self):
        ...
        if path.path == "/cgi-bin/externalcontact/add_msg_template":
            ...
            assert payload.get("external_userid") == ["external-1"], payload
            assert payload.get("text", {}).get("content") == "Go定时客户群发", payload
            self.json_response({"errcode": 0, "errmsg": "ok", "msgid": "msg-contact-scheduled"})
            return
        self.send_response(404)
        self.end_headers()
...
TODAY="$(date +%F)"
DUE_TIME="${MOCHAT_CONTACT_BATCH_SEND_DUE_TIME:-2000-01-01 00:00:00}"
...
DELETE FROM mc_work_contact_employee WHERE id = 1;
DELETE FROM mc_work_contact WHERE id = 1;
...
INSERT INTO mc_corp (
  ...
  1, 'Go定时群发企业', 'ww-scheduled', 'employee-secret', 'contact-secret',
...
INSERT INTO mc_work_contact (
  ...
  1, 1, 'external-1', 'Go客户', 1, 1, '$TODAY 08:15:00', '$TODAY 08:15:00', NULL
);

INSERT INTO mc_work_contact_employee (
  id, employee_id, contact_id, add_way, oper_userid, state, corp_id, status, create_time, created_at, updated_at, deleted_at
) VALUES (
  1, 1, 1, 1, 'employee-wx', '', 1, 1, '$TODAY 08:16:00', '$TODAY 08:16:00', '$TODAY 08:16:00', NULL
);
...
INSERT INTO mc_contact_message_batch_send (
  ...
  1, 1, 1, 'Go管理员', JSON_ARRAY(1), JSON_OBJECT(), JSON_OBJECT(), JSON_ARRAY(JSON_OBJECT('msgType', 'text', 'content', 'Go定时客户群发')),
...
env -u GOROOT \
  ...
  MOCHAT_GO_ENABLE_CONTACT_BATCH_SEND_CRON=1 \
  MOCHAT_GO_CONTACT_BATCH_SEND_CRON_RUN_ON_START=1 \
  MOCHAT_GO_CONTACT_BATCH_SEND_CRON_INTERVAL_SECONDS=60 \
  "$GO_BIN" >"$GO_LOG" 2>&1 &
GO_PID="$!"
...
task = tasks.get("cron-contact-batch-send")
assert task, status
assert task.get("status") == "running", task
assert task.get("started_at"), task
PY

wait_mysql_scalar "SELECT COUNT(*) FROM mc_contact_message_batch_send_employee WHERE batch_id = 1 AND employee_id = 1 AND wx_user_id = 'employee-wx' AND status = 1 AND receive_status = 1 AND msg_id = 'msg-contact-scheduled'" "1"
wait_mysql_scalar "SELECT COUNT(*) FROM mc_contact_message_batch_send_result WHERE batch_id = 1 AND employee_id = 1 AND contact_id = 1 AND external_user_id = 'external-1'" "1"
wait_mysql_scalar "SELECT CONCAT(send_status, '|', send_employee_total, '|', send_contact_total, '|', send_total, '|', not_send_total) FROM mc_contact_message_batch_send WHERE id = 1" "1|1|1|0|1"

grep -q "go cron enabled: ContactMessageBatchSend 定时发送" "$GO_LOG"
grep -q "ContactMessageBatchSend scheduled cron finished: scanned=1 sent=1 skipped=0 failed=0" "$GO_LOG"

echo "ContactMessageBatchSend scheduled cron smoke passed"
