...
PROJECT_NAME="${MOCHAT_STACK_PROJECT:-mochat-go-mark-tags-check}"
GO_ADDR="${MOCHAT_GO_ADDR:-127.0.0.1:18097}"
MYSQL_PORT="${MOCHAT_MYSQL_PORT:-13341}"
REDIS_PORT="${MOCHAT_REDIS_PORT:-26399}"
WECOM_ADDR="${MOCHAT_WECOM_ADDR:-127.0.0.1:19079}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mochat-go-mark-tags.XXXXXX")"
...
DELETE FROM mc_contact_employee_track WHERE corp_id = 1 AND contact_id = 101 AND employee_id = 3;
DELETE FROM mc_work_contact_tag_pivot WHERE contact_id = 101 AND employee_id = 3;
DELETE FROM mc_work_contact_tag WHERE id IN (201, 202);
DELETE FROM mc_work_contact_employee WHERE id = 301;
DELETE FROM mc_work_contact WHERE id = 101;
DELETE FROM mc_work_employee WHERE id = 3;
...
INSERT INTO mc_corp (
  ...
  1, 'Go打标签企业', 'ww-mark-tags', 'employee-secret', 'contact-secret',
...
INSERT INTO mc_work_employee (
  ...
  3, 'go-employee', 1, 'Go打标员工', '13600000003', '', 1, '',
  '', '', '', '', JSON_OBJECT(), 1, '',
  JSON_OBJECT(), '', '', 'open-go-employee',
...
printf '[1,101,3,[201,202]]' >"$WORK_DIR/event.json"
compose exec -T redis redis-cli -x RPUSH mochat-go:mark-tags <"$WORK_DIR/event.json" >/dev/null

wait_mysql_scalar "SELECT COUNT(*) FROM mc_work_contact_tag_pivot WHERE contact_id = 101 AND employee_id = 3 AND contact_tag_id = 201 AND type = 1 AND deleted_at IS NULL" "1"
wait_mysql_scalar "SELECT COUNT(*) FROM mc_work_contact_tag_pivot WHERE contact_id = 101 AND employee_id = 3 AND contact_tag_id = 202 AND type = 1 AND deleted_at IS NULL" "1"
wait_mysql_scalar "SELECT COUNT(*) FROM mc_contact_employee_track WHERE corp_id = 1 AND contact_id = 101 AND employee_id = 3 AND event = 2 AND content = '系统对该客户打标签【重点】' AND deleted_at IS NULL" "1"
wait_mysql_scalar "SELECT COUNT(*) FROM mochat_go_background_task_executions WHERE tenant_id = 1 AND task_name = 'mark-tags' AND kind = 'queue_item' AND status = 'succeeded' AND execution_id <> '' AND task_run_id <> '' AND started_at IS NOT NULL AND stopped_at IS NOT NULL" "1"
...
body = events[0]["body"]
assert body["userid"] == "go-employee", body
assert body["external_userid"] == "external-101", body
assert body["add_tag"] == ["wx-tag-201"], body
PY

grep -q "go worker enabled: MarkTags Redis consumer" "$GO_LOG"

echo "mark tags worker smoke passed"
