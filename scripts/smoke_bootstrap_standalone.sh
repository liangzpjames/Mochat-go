...
test -n "$USER_ID"
test -n "$ROLE_ID"
CORP_ID="$(mysql_scalar "SELECT id FROM mc_corp WHERE tenant_id = $TENANT_ID AND deleted_at IS NULL ORDER BY id ASC LIMIT 1")"
test -n "$CORP_ID"
test "$(mysql_scalar "SELECT COUNT(*) FROM mc_corp WHERE tenant_id = $TENANT_ID AND deleted_at IS NULL")" = "1"
test "$(mysql_scalar "SELECT COUNT(*) FROM mc_work_employee WHERE corp_id = $CORP_ID AND log_user_id = $USER_ID AND deleted_at IS NULL")" = "1"
