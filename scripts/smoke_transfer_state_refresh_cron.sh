...
PROJECT_NAME="${MOCHAT_STACK_PROJECT:-mochat-go-transfer-state-cron-check}"
GO_ADDR="${MOCHAT_GO_ADDR:-127.0.0.1:18090}"
MYSQL_PORT="${MOCHAT_MYSQL_PORT:-13322}"
REDIS_PORT="${MOCHAT_REDIS_PORT:-26393}"
WECOM_ADDR="${MOCHAT_WECOM_ADDR:-127.0.0.1:19056}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mochat-go-transfer-state-cron.XXXXXX")"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path.path == "/cgi-bin/externalcontact/transfer_result":
            assert payload.get("handover_userid") == "handover-user", payload
            assert payload.get("takeover_userid") == "takeover-user", payload
            self.json_response({
                "errcode": 0,
                "errmsg": "ok",
                "customer": [
                    {"external_userid": "external-other", "status": 2},
                    {"external_userid": "external-31", "status": 3}
                ]
            })
            return
        self.send_response(404)
...
DELETE FROM mc_work_transfer_log WHERE id = 1;
...
INSERT INTO mc_corp (
  ...
  1, 'Go转接企业', 'ww-transfer', 'employee-secret', 'contact-secret',
  'callback-token', 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG', 1, '$TODAY 08:00:00', '$TODAY 08:00:00', NULL
);

INSERT INTO mc_work_transfer_log (
  id, corp_id, status, type, name, contact_id,
  handover_employee_id, takeover_employee_id, state, created_at, updated_at
) VALUES (
  1, 1, 2, 1, '转接客户', 'external-31',
  'handover-user', 'takeover-user', 2, '$TODAY 08:10:00', '$TODAY 08:10:00'
);
SQL

redis_scalar DEL log_id >/dev/null
...
env -u GOROOT \
  ...
  MOCHAT_GO_ENABLE_TRANSFER_STATE_REFRESH_CRON=1 \
  MOCHAT_GO_TRANSFER_STATE_REFRESH_CRON_RUN_ON_START=1 \
  MOCHAT_GO_TRANSFER_STATE_REFRESH_CRON_INTERVAL_SECONDS=60 \
  "$GO_BIN" >"$GO_LOG" 2>&1 &
GO_PID="$!"
...
task = tasks.get("cron-transfer-state-refresh")
assert task, status
assert task.get("status") == "running", task
assert task.get("started_at"), task
PY

wait_mysql_scalar "SELECT state FROM mc_work_transfer_log WHERE id = 1" "3"
wait_redis_scalar "1" GET log_id

grep -q "go cron enabled: TransferStateRefresh" "$GO_LOG"
grep -q "TransferStateRefresh cron refreshed: log=1" "$GO_LOG"

echo "TransferStateRefresh cron smoke passed"
