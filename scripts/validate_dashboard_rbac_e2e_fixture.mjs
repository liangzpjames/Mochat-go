...
export function validateFixture(fixture, manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'))) {
  ...
  for (const name of requiredAccounts) {
    ...
    if (name === 'roleDisabledDirectRetained' && (!account.disabledRolePermissionCode || typeof account.disabledRoleId !== 'number' || !account.directRetainedPermissionCode || !account.expectedSources?.some((source) => source.code === account.directRetainedPermissionCode && source.type === 'direct') || !account.forbiddenCodes?.includes(account.disabledRolePermissionCode) || !account.forbiddenRoleIds?.includes(account.disabledRoleId))) throw new Error('roleDisabledDirectRetained requires disabled code/role denial and retained direct source');
  }
