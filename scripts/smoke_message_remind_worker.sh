...
PROJECT_NAME="${MOCHAT_STACK_PROJECT:-mochat-go-message-remind-check}"
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mochat-go-message-remind.XXXXXX")"
...
DELETE FROM mochat_go_background_task_executions WHERE task_name = 'message-remind';
DELETE FROM mochat_go_background_task_runs WHERE name = 'message-remind';
DELETE FROM mochat_go_background_tasks WHERE name = 'message-remind';
DELETE FROM mc_work_agent WHERE id = 1001 OR corp_id = 1;
...
INSERT INTO mc_corp (
  ...
  1, 'Go消息提醒企业', 'ww-message-remind', 'employee-secret', 'contact-secret',
...
INSERT INTO mc_work_agent (
  ...
  1001, 1, '100001', 'agent-secret', '提醒应用', '', '', 0,
  '', 0, 0, '', NOW(), NOW(), NULL
);
SQL
...
env -u GOROOT \
  ...
  MOCHAT_GO_ENABLE_MESSAGE_REMIND_WORKER=1 \
...
{"corpId":1,"toUser":["go-user-a","go-user-b"],"msgType":"text","content":"独立Go消息提醒","extra":{"safe":1,"enable_duplicate_check":1,"duplicate_check_interval":600},"source":"smoke"}
JSON
compose exec -T redis redis-cli -x RPUSH mochat-go:message-remind <"$WORK_DIR/event.json" >/dev/null

wait_file_line_count "$WECOM_EVENTS" 1
wait_mysql_scalar "SELECT COUNT(*) FROM mochat_go_background_task_executions WHERE tenant_id = 1 AND task_name = 'message-remind' AND kind = 'queue_item' AND status = 'succeeded' AND execution_id <> '' AND task_run_id <> '' AND started_at IS NOT NULL AND stopped_at IS NOT NULL" "1"
...
wait_redis_scalar "0" LLEN mochat-go:message-remind
wait_redis_scalar "0" LLEN mochat-go:message-remind:processing
wait_redis_scalar "0" LLEN mochat-go:message-remind:dead
...
body = events[0]["body"]
assert body["touser"] == "go-user-a|go-user-b", body
assert body["msgtype"] == "text", body
assert body["agentid"] == "100001", body
assert body["safe"] == 1, body
assert body["enable_duplicate_check"] == 1, body
assert body["duplicate_check_interval"] == 600, body
assert body["text"]["content"] == "独立Go消息提醒", body
PY

grep -q "go worker enabled: MessageRemind Redis consumer" "$GO_LOG"

echo "message remind worker smoke passed"
