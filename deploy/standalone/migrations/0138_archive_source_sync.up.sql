...
SET @archive_runs_invalid := (
  ...
  AND (
    ...
    OR (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'mochat_go_archive_sync_runs' AND (
      ...
      OR (column_name = 'cursor_token' AND data_type = 'varchar' AND character_maximum_length = 255 AND is_nullable = 'NO' AND (column_default = '' OR column_default = CONCAT(CHAR(39),CHAR(39)) OR column_default = CONCAT(CHAR(34),CHAR(34))) AND REPLACE(LOWER(COALESCE(extra,'')), 'default_generated', '') = '')
      ...
      OR (column_name = 'error_code' AND data_type = 'varchar' AND character_maximum_length = 96 AND is_nullable = 'NO' AND (column_default = '' OR column_default = CONCAT(CHAR(39),CHAR(39)) OR column_default = CONCAT(CHAR(34),CHAR(34))) AND REPLACE(LOWER(COALESCE(extra,'')), 'default_generated', '') = '')
      ...
      OR (column_name = 'lease_token' AND data_type = 'varchar' AND character_maximum_length = 128 AND is_nullable = 'NO' AND (column_default = '' OR column_default = CONCAT(CHAR(39),CHAR(39)) OR column_default = CONCAT(CHAR(34),CHAR(34))) AND REPLACE(LOWER(COALESCE(extra,'')), 'default_generated', '') = '')
      OR (column_name IN ('started_at','finished_at','lease_expires_at','heartbeat_at') AND data_type = 'datetime' AND datetime_precision = 6 AND is_nullable = 'YES' AND (column_default IS NULL OR UPPER(TRIM(COALESCE(column_default,''))) = 'NULL') AND REPLACE(LOWER(COALESCE(extra,'')), 'default_generated', '') = '')
