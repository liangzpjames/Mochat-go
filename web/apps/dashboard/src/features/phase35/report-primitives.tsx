...
export function ReportPrimitives({result,itemColumns}:{result?:ReportResult|undefined;itemColumns?:DetailColumn[]}) {
  ...
  return <><MetricCardGrid metrics={result.summary??{}}/>{Boolean(result.series?.length)&&<section aria-label="趋势"><h2>趋势</h2><SimpleTrendChart points={result.series!.map((point)=>({at:text(point.at??point.day),value:Number(point.value??0)}))}/></section>}{dimensions.length>0&&<section aria-label="分布"><h2>分布</h2><MetricCardGrid items={dimensions.map((item)=>({label:item.label,value:item.value}))}/></section>}<BusinessDataNotice visible={Boolean(result.limitations?.length)} />{items.length?<ReportDetailTable items={items} {...pagination} {...(itemColumns?{columns:itemColumns}:{})}/>:!result.limitations?.length&&<p role="status">当前筛选范围暂无明细数据。</p>}</>;
}
