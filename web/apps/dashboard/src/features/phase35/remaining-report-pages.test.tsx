...
test('employee limitations use plain-language guidance',async()=>{const api={read:vi.fn().mockResolvedValue({limitations:[{provider:'conversation_archive',message:'会话 Provider 未提供'}]}),write:vi.fn()};render(wrap(<EmployeeReportPage api={api}/>));expect(await screen.findByText(/完成会话存档配置并同步数据后/)).not.toBeNull();expect(screen.queryByText(/Provider/i)).toBeNull()});
test('behavior maps nested pagination and renders Chinese business detail',async()=>{const api={read:vi.fn().mockResolvedValue({summary:{behavior:3},items:[{eventType:'setting.updated',actorId:4,objectId:'setting-1',occurredAt:'2026-08-06T00:00:00Z',detail:'客户来源已更新'}],pagination:{page:1,pageSize:20,total:3}}),write:vi.fn()};render(wrap(<BehaviorReportPage api={api}/>));const pagination=await screen.findByRole('navigation',{name:'分页'});expect(pagination.textContent).toContain('共 3 条');expect(pagination.textContent).toContain('第 1/1 页');expect(screen.getByText('设置更新')).not.toBeNull();expect(screen.getByText('账号 4')).not.toBeNull();expect(screen.queryByText('Event Type')).toBeNull();fireEvent.click(screen.getByRole('button',{name:'查看行为详情'}));expect(screen.getByRole('dialog',{name:'行为详情'})).not.toBeNull()});
...
test('summary report does not expose raw limitation details',async()=>{const api={read:vi.fn().mockResolvedValue({summary:{},limitations:[{provider:'orders',message:'订单 Provider 未提供'}]}),write:vi.fn()};render(wrap(<ReportPage api={api}/>));expect(await screen.findByText('部分数据暂未同步完整，当前仅展示已获取的数据。')).not.toBeNull();expect(screen.queryByText(/Provider/i)).toBeNull()});
