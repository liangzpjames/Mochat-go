    ...
    <Phase35PageShell title="员工权限" description="员工账号、启用/停用状态与角色分配" actions={<button type="button" onClick={(event) => { triggerRef.current = event.currentTarget; openCreate(); }}>新增员工</button>}>
      <div className="phase35-page">
        ...
        <section className="phase35-kpis" aria-label="员工指标">
          <article className="phase35-kpi phase35-kpi-primary"><span>员工总数</span><strong>{total}</strong><small>筛选条件下员工数量</small></article>
          <article className="phase35-kpi phase35-kpi-green"><span>正常</span><strong>{query.data?.normalNum ?? 0}</strong><small>状态为正常的员工</small></article>
          <article className="phase35-kpi phase35-kpi-red"><span>停用</span><strong>{query.data?.disableNum ?? 0}</strong><small>状态为停用的员工</small></article>
        </section>

        <section className="phase35-card phase35-table-card">
          <header className="phase35-card-header"><div><h2>员工列表</h2><p>启用/停用与角色分配</p></div><span className="phase35-chip">共 {total} 条，第 {page}/{totalPage} 页</span></header>
          <Phase35DataState loading={query.isLoading} error={query.isError} empty={!items.length} emptyContent={<p className="phase35-empty">暂无员工</p>} onRetry={() => void query.refetch()}>
            <div className="phase35-table">
              <table>
                <thead><tr><th>姓名</th><th>手机号</th><th>角色</th><th>状态</th><th>操作</th></tr></thead>
                <tbody>
                  {items.map((item) => (
                    <tr key={item.userId}>
