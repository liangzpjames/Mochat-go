...
import {
  createMenuAdminApi,
  type MenuNode,
} from "../menu-admin/menu-admin-api";
import type { AccessAudit, Page } from "../access/access-admin-api";
...
function authorizationName(node: MenuNode): string {
  return (
    authorizationNameMap[node.name] ??
    (/\?{2,}|provider/i.test(node.name)
      ? `未命名权限（${node.menuId}）`
      : node.name || `未命名权限（${node.menuId}）`)
  );
}
      ...
      {nodes.map((node) => (
        <li key={node.menuId}>
          ...
          <span className="phase35-chip">
            {node.status === 1 ? "已授权" : "已停用"}
          </span>
          <ConfirmAction
            title={
              node.status === 1
                ? `确认停用授权节点“${authorizationName(node)}”？`
                : `确认启用授权节点“${authorizationName(node)}”？`
            }
            {...(node.status === 1
              ? { description: "停用后该菜单不再授予当前企业用户。" }
              : {})}
            onConfirm={() => onToggle(node.menuId)}
          >
            <button type="button">
              {node.status === 1
                ? `停用 ${authorizationName(node)}`
                : `启用 ${authorizationName(node)}`}
            </button>
          </ConfirmAction>
...
function LegacyCompanyAuthorizationPage({ api }: { api: MenuAdminApi }) {
  ...
  const query = useQuery({
    ...
    queryFn: () => api.list({ name: "", page: 1, perPage: 200 }),
  });
  const flat = query.data?.list ?? [];
  const nodes = buildMenuTree(flat);
  const count = countNodes(nodes);
  const toggle = useMutation({
    mutationFn: (input: { id: number; next: number }) =>
      api.updateStatus(input.id, input.next),
    onSuccess: () =>
  ...
  return (
    <Phase35PageShell
      title="授权管理"
      description="按菜单树查看授权状态；此处以“切换状态”演示授权开关（停用后该菜单不再授予）"
      actions={<span className="phase35-chip">共 {count} 个节点</span>}
    >
      <div className="phase35-page">
        <section className="phase35-card">
          ...
          <Phase35DataState
            ...
            error={query.isError}
            empty={!nodes.length}
            emptyContent={<p className="phase35-empty">暂无授权菜单</p>}
          ...
          >
            <OptionTree
              ...
              onToggle={(id) => {
                ...
                if (node)
                  toggle.mutate({ id, next: node.status === 1 ? 0 : 1 });
              }}
            />
          </Phase35DataState>
        </section>
...
function AuditPage({ api }: { api: AuditApi }) {
  ...
  const query = useQuery({
    queryKey: ["dashboard-access-audits", page],
    queryFn: () => api.audits({ page, perPage: 50 }),
  });
  return (
    <Phase35PageShell
      title="权限审计"
      description="同事务记录的授权变更与 expectedVersion 结果"
    >
      <div className="phase35-page">
        <section className="phase35-card phase35-table-card">
          <Phase35DataState
            loading={query.isLoading}
            error={query.isError}
            empty={!query.data?.list.length}
            onRetry={() => void query.refetch()}
            emptyContent={<p>暂无审计记录</p>}
          >
            <div className="dashboard-table-scroll">
              <table>
                ...
                <tbody>
                  {(query.data?.list ?? []).map((audit) => (
                    <tr key={audit.id}>
                      <td>{audit.time}</td>
                      <td>{audit.actorName || audit.actorUserId || "-"}</td>
                      <td>{audit.action}</td>
                      <td>
                        {audit.targetType} {audit.targetId}
                      </td>
                      <td>{audit.requestId}</td>
                      <td>{audit.resultVersion ?? "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export function CompanyAuthorizationPage({
  api,
}: {
  api: MenuAdminApi | AuditApi;
}) {
  return "audits" in api ? (
    <AuditPage api={api} />
  ) : (
    <LegacyCompanyAuthorizationPage api={api} />
  );
}

function buildMenuTree(flat: MenuNode[]): MenuNode[] {
  const byId = new Map<number, MenuNode>(
    flat.map((node) => [node.menuId, { ...node, children: [] }]),
  );
