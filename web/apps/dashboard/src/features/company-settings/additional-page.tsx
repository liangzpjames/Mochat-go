...
function LegacyCompanyAdditionalPage({ api }: { api: MenuAdminApi }) {
  ...
  const query = useQuery({
    queryKey: ["company-additional", keyword, page],
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      setCreating(false);
      setError("");
      void queryClient.invalidateQueries({ queryKey: ["company-additional"] });
    },
  ...
  return (
    <Phase35PageShell
      title="附加权限"
      description="维护菜单/功能点，作为附加权限的来源"
      actions={
        <button
          type="button"
          onClick={(event) => {
      ...
      <div className="phase35-page">
        ...
        <section className="phase35-card phase35-table-card">
          ...
          <Phase35DataState
            ...
            error={query.isError}
            empty={!items.length}
            emptyContent={<p className="phase35-empty">暂无菜单</p>}
                  ...
                  {items.map((item) => (
                    <tr key={item.menuId}>
                      ...
                      <td>
                        ...
                        <ConfirmAction
                          title={`确认删除菜单“${item.name}”？`}
                          description="删除后引用该菜单的授权将失效。"
                          onConfirm={() => remove.mutate(item.menuId)}
                        >
...
function CatalogPage({ api }: { api: CatalogApi }) {
  const query = useQuery({
    queryKey: ["dashboard-access-catalog"],
    queryFn: api.catalog,
  });
  return (
    <Phase35PageShell
      title="权限目录"
      description="53 个 Dashboard 页面及其 scopeRequired 治理定义"
    >
      <div className="phase35-page">
        <section className="phase35-card phase35-table-card">
          <Phase35DataState
                ...
                <tbody>
                  {(query.data ?? []).map((item) => (
                    <tr key={item.code}>
                      ...
                      <td>
                        {item.scopeRequired ? "需要员工数据范围" : "企业级"}
                      </td>
                      <td>{item.superadminOnly ? "是" : "否"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export function CompanyAdditionalPage({
  api,
}: {
  api: MenuAdminApi | CatalogApi;
}) {
  return "catalog" in api ? (
    <CatalogPage api={api} />
  ) : (
    <LegacyCompanyAdditionalPage api={api} />
  );
}
