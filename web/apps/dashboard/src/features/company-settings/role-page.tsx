...
function AccessRolePage({ api }: { api: AccessRoleApi }) {
  ...
  const save = useMutation({ mutationFn: () => editing ? api.updateRole(editing.id, { name: name.trim(), remark: remark.trim(), permissions: [], expectedVersion: editing.version }) : api.createRole({ name: name.trim(), remark: remark.trim(), status: 1, permissions: [] }), onSuccess: () => { setEditing(null); void queryClient.invalidateQueries({ queryKey: ['dashboard-access-roles'] }); }, onError: (reason) => setError(reason instanceof Error ? reason.message : '保存失败') });
  ...
  return <Phase35PageShell title="角色管理" description="角色完整 CRUD、启停与成员约束" actions={<button type="button" onClick={() => { setEditing({ id: 0, name: '', remark: '', status: 1, isSystem: false, memberCount: 0, permissions: [], version: 1 }); setName(''); setRemark(''); }}>新建角色</button>}><div className="phase35-page"><section className="phase35-card phase35-table-card"><table><thead><tr><th>角色</th><th>状态</th><th>成员</th><th>操作</th></tr></thead><tbody>{(query.data?.list ?? []).map((role) => <tr key={role.id}><td>{role.name}<br /><small>{role.remark}</small></td><td>{role.status === 1 ? '启用' : '停用'}</td><td>{role.memberCount}</td><td><button type="button" onClick={() => { setEditing(role); setName(role.name); setRemark(role.remark); }}>编辑</button><ConfirmAction title={`${role.status === 1 ? '停用' : '启用'}角色“${role.name}”？`} onConfirm={() => toggle.mutate(role)}><button type="button">{role.status === 1 ? '停用' : '启用'}</button></ConfirmAction><ConfirmAction title={`确认删除角色“${role.name}”？`} description={role.memberCount > 0 ? '有成员时服务端将返回 409。' : undefined} onConfirm={() => remove.mutate(role)}><button type="button">删除</button></ConfirmAction></td></tr>)}</tbody></table>{error && <p role="alert">{error}</p>}</section>{editing && <DashboardDialog open title={editing.id ? '编辑角色' : '新建角色'} confirmText="保存" confirmLoading={save.isPending} onCancel={() => setEditing(null)} onConfirm={() => save.mutate()}><label>角色名称<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>备注<input value={remark} onChange={(event) => setRemark(event.target.value)} /></label></DashboardDialog>}</div></Phase35PageShell>;
}
