...
type Props = { data: ConversationTrajectoryDay | undefined; date: string; conversationType: ConversationTrajectoryType; isLoading: boolean; onDateChange(date: string): void; onTypeChange(type: ConversationTrajectoryType): void; onRefresh(): void; onOpenEvent(event: ConversationTrajectoryEvent, button: HTMLButtonElement): void };
...
export function ConversationTrajectoryTimeline(props: Props) {
  ...
  useEffect(() => {
    if (!props.data || !scrollRef.current) return;
    const earliest = [...eventsByHour.keys()].sort()[0];
    const target = document.getElementById(`trajectory-hour-${earliest && earliest < '08' ? earliest : '08'}`);
  ...
  const moveDate = (delta: number) => {
    const base = new Date(`${currentDate}T00:00:00+08:00`);
    props.onDateChange(new Date(base.getTime() + delta * 86400000).toISOString().slice(0, 10));
  };
  return <section className="conversation-trajectory-main" aria-label="会话轨迹时间轴">
    <header className="conversation-trajectory-toolbar">
      ...
      <button type="button" onClick={props.onRefresh} disabled={props.isLoading}>刷新</button>
    </header>
