...
export function ConversationExportPage({ api }: { api: ConversationGlobalApi }) {
  ...
  return <section className="conversation-export-page">
    <header className="conversation-global-header dashboard-data-card"><div><p className="conversation-global-eyebrow">会话存档</p><h1>会话导出</h1><p>导出当前查询结果页；异步任务和受控下载接口接入后再扩展全量导出。</p></div><button type="button" disabled={query.isFetching} onClick={() => void query.refetch()}>刷新</button></header>
    ...
    {archiveUnauthorized && <PageState state="forbidden" title="当前企业未开通会话内容存档" description="请先完成会话存档授权后再导出。" />}
