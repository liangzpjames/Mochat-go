...
function csvCell(value: unknown): string {
  const text = typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean' || typeof value === 'bigint' ? String(value) : '';
...
export function ConversationExportPage({ api }: { api: ConversationGlobalApi }) {
  ...
  const archiveUnauthorized = isConversationArchiveUnavailable(query.error);
  ...
  return <section className="conversation-export-page">
    ...
    {archiveUnauthorized && <ConversationArchiveUnavailableState />}
