...
describe('ContactPage', () => {
  ...
  it('opens aggregate detail and executes lifecycle actions', async () => {
    const api = { listContacts: vi.fn().mockResolvedValue({ items: [contact], nextCursor: '' }), getContact: vi.fn().mockResolvedValue(detail), updateAssignment: vi.fn().mockResolvedValue(detail.assignment), bindTags: vi.fn().mockResolvedValue(undefined), listFollowUps: vi.fn().mockResolvedValue({ items: detail.followUps, nextCursor: '' }), appendFollowUp: vi.fn().mockResolvedValue({}), releaseToPublicPool: vi.fn().mockResolvedValue({ ...detail.assignment, status: 'public_pool', version: 5 }), createOpportunity: vi.fn().mockResolvedValue({}) } as any;
