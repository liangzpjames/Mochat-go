...
import type { PublicPoolAction, PublicPoolAssignment, PublicPoolListInput, ScrmApi } from './scrm-api';
...
export function PublicPoolPage({ api }: { api: PublicPoolApi }) {
  ...
  const claim = useMutation({
    mutationFn: (item: PublicPoolAssignment) => api.claimFromPublicPool({ corpId, contactId: item.contactId, userId, version: item.version, idempotencyKey: `claim-${item.contactId}-${item.version}-${userId}` }),
    onSuccess: () => { setFeedback('领取成功，联系人已转入我的负责范围。'); void refresh(); },
  });
  const batch = useMutation({
    mutationFn: () => api.batchClaimFromPublicPool({ corpId, userId, targets: Object.entries(selected).map(([contactId, version]) => ({ contactId, version, idempotencyKey: `batch-claim-${contactId}-${version}-${userId}` })) }),
    onSuccess: ({ results }) => { const failed = results.filter((result) => result.status === 'failed'); setFeedback(failed.length === 0 ? `批量领取完成：${results.length} 成功。` : `批量领取完成：${results.length - failed.length} 成功，${failed.length} 失败（${[...new Set(failed.map((result) => result.errorCode))].join('、')}）。`); setSelected({}); void refresh(); },
  });
  const move = useMutation({
    mutationFn: (action: PublicPoolAction) => api.releaseToPublicPool({ corpId, contactId: operationContactId.trim(), version: Number(operationVersion), action, reason: operationReason.trim(), idempotencyKey: `pool-${action}-${operationContactId.trim()}-${Number(operationVersion)}-${userId}` }),
    onSuccess: () => { setFeedback('公海归属已更新，历史负责人和原因已留痕。'); setOperationContactId(''); setOperationVersion(''); setOperationReason(''); void refresh(); },
  });
  const canEdit = access.allowedActions.has('/customer/contact@edit');
  const mutationError = claim.error ?? batch.error ?? move.error;
  ...
  return <section className="scrm-public-pool-page">
    ...
    {mutationError && <PageState state={pageStateForError(mutationError)} description="公海操作未完成，请刷新数据后重试。" onRetry={() => void query.refetch()} />}
  </section>;
}
