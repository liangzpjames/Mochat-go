...
type PublicPoolApi = Pick<ScrmApi, 'listPublicPool' | 'claimFromPublicPool' | 'batchClaimFromPublicPool' | 'releaseToPublicPool' | 'listContactOptions'>;
...
export function PublicPoolPage({ api }: { api: PublicPoolApi }) {
  ...
  const [feedback, setFeedback] = useState(''); const [operationContactId, setOperationContactId] = useState(''); const [operationReason, setOperationReason] = useState('');
  ...
  const operationContact = (contacts.data ?? []).find((item) => item.id === operationContactId);
  ...
  return <section className="scrm-public-pool-page">
    <header className="scrm-page-header dashboard-page-header dashboard-data-card"><div><p className="scrm-page-eyebrow">SCRM · 客户流转</p><h1>客户公海</h1><p>统一筛选、回收与领取客户；版本、幂等和企业范围共同保护每次归属变更。</p></div><span className="scrm-page-badge">归属变更全留痕</span></header>
    <div aria-label="公海数据概览" className="dashboard-stat-grid scrm-overview" role="region"><article><span>当前结果</span><strong>{query.data?.items.length ?? 0}</strong><small>本页真实数据</small></article><article><span>已选择</span><strong>{Object.keys(selected).length}</strong><small>批量领取范围</small></article><article><span>本页回收次数</span><strong>{query.data?.items.reduce((sum, item) => sum + item.recycleCount, 0) ?? 0}</strong><small>历史流转累计</small></article><article><span>可领取状态</span><strong>{query.data?.items.filter((item) => item.status === 'public_pool').length ?? 0}</strong><small>等待重新分配</small></article></div>
    ...
    {canEdit && <div className="dashboard-data-card"><div className="dashboard-card-heading"><div><h2>纳入公海</h2><p>进入、退回和管理员回收共用一套原子变更与审计历史。</p></div></div><div className="dashboard-filter-bar"><label>联系人<select aria-label="操作联系人" value={operationContactId} onChange={(event) => setOperationContactId(event.target.value)}><option value="">选择联系人</option>{(contacts.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>原因<input aria-label="操作入海原因" value={operationReason} onChange={(event) => setOperationReason(event.target.value)} /></label><div className="dashboard-table-actions">{([['enter', '进入公海'], ['return', '退回公海'], ['reclaim', '管理员回收']] as const).map(([action, label]) => <button key={action} type="button" disabled={!operationContact || !operationContact.version || !operationReason.trim() || move.isPending} onClick={() => { if (!operationContact?.version) return; const contactId = operationContact.id; const version = operationContact.version; move.mutate({ corpId, contactId, version, action, reason: operationReason.trim(), idempotencyKey: `pool-${action}-${contactId}-${version}-${userId}` }); }}>{label}</button>)}</div></div></div>}
    {feedback && <p role="alert" className="dashboard-inline-feedback">{feedback}</p>}
    <div className="dashboard-data-card scrm-list-card"><div className="dashboard-card-heading"><div><h2>公海客户</h2><p>展示来源、标签、前负责人、入海原因、回收次数和最后跟进时间。</p></div><div className="dashboard-table-actions"><button aria-label="刷新公海" disabled={query.isFetching} onClick={() => void query.refetch()} type="button">刷新</button>{canEdit && <button type="button" disabled={Object.keys(selected).length === 0 || batch.isPending} onClick={() => { setFeedback(''); batch.mutate({ corpId, userId, targets: Object.entries(selected).map(([contactId, version]) => ({ contactId, version, idempotencyKey: `batch-claim-${contactId}-${version}-${userId}` })) }); }}>批量领取</button>}</div></div>
      ...
      {(cursor || query.data?.nextCursor) && <DashboardCursorPagination cursor={cursor} nextCursor={query.data?.nextCursor} onCursorChange={(nextCursor) => setSearchParams(poolSearch(searchParams, applied, nextCursor))} />}
    </div>
