...
export function PublicPoolPage({ api }: { api: Pick<ScrmApi, 'listPublicPool' | 'claimFromPublicPool'> }) {
  ...
  return <section className="scrm-public-pool-page"><header><h1>客户公海</h1><p>领取后将成为当前账号负责的客户。</p></header><table><thead><tr><th>客户</th><th>状态</th><th>版本</th><th>操作</th></tr></thead><tbody>{query.data.items.map((item) => <tr key={item.id}><td>{item.contactId}</td><td>{item.status}</td><td>{item.version}</td><td><button type="button" disabled={claim.isPending} onClick={() => claim.mutate({ contactId: item.contactId, version: item.version })}>领取</button></td></tr>)}</tbody></table>{claim.isError && <p role="alert">领取冲突，请刷新后重试。</p>}</section>;
}
