...
export function OpportunityPage({ api }: { api: OpportunityApi }) {
  ...
  return <section className="scrm-opportunity-page">
    ...
    <div className="dashboard-data-card scrm-list-card"><div className="dashboard-card-heading"><div><h2>商机列表</h2><p>筛选与游标保存在 URL，刷新和前进后退均可恢复。</p></div><button aria-label="刷新商机" disabled={opportunities.isFetching} onClick={() => void opportunities.refetch()} type="button">刷新</button></div>
