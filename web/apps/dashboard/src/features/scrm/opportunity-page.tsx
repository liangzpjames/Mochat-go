...
export function OpportunityPage({ api }: { api: Pick<ScrmApi, 'listOpportunities' | 'changeOpportunityStage'> }) {
  ...
  const changeState = change.isError ? pageStateForError(change.error) : null;
  return <section><header className="dashboard-page-header"><h1>机会</h1></header><div className="dashboard-filter-bar"><label>阶段<select aria-label="阶段" value={stage} onChange={(event) => setStage(event.target.value)}><option value="">全部</option><option value="proposal">意向</option><option value="won">赢单</option><option value="lost">输单</option></select></label></div><div className="dashboard-data-card"><div className="dashboard-table-scroll"><table><thead><tr><th>联系人</th><th>阶段</th><th>金额</th><th>操作</th></tr></thead><tbody>{query.data.items.map((item) => <tr key={item.id}><td>{item.contactId}</td><td>{item.stage}</td><td>{item.amount}</td><td><button type="button" disabled={item.status !== 'open'} onClick={() => change.mutate({ opportunityId: item.id, version: item.version, toStage: 'won' })}>赢单</button></td></tr>)}</tbody></table></div></div>{changeState === 'conflict' && <PageState description="机会状态冲突，请刷新后重试" state="conflict" />}{changeState === 'error' && <p role="alert">机会状态冲突，请刷新后重试</p>}</section>;
}
