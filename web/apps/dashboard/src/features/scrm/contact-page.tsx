...
export function ContactPage({ api }: { api: ContactApi }) {
  ...
  return <section className="scrm-contact-page">
    ...
    {contactId && <div className="dashboard-data-card scrm-contact-detail"><div className="dashboard-card-heading"><div><h2>联系人详情</h2><p>资料与生命周期信息来自同一 tenant/corp 聚合接口。</p></div><button type="button" onClick={() => setSearchParams(nextSearch(searchParams, applied, cursor))}>关闭</button></div>
      {detail.isPending ? <PageState state="loading" /> : detail.isError ? <PageState state={pageStateForError(detail.error)} onRetry={() => void detail.refetch()} /> : <ContactDetailPanel value={detail.data} api={api} corpId={corpId} canEdit={canEdit} tagId={tagId} setTagId={setTagId} bindTag={() => bindTag.mutate()} poolReason={poolReason} setPoolReason={setPoolReason} release={() => release.mutate(detail.data)} amount={amount} setAmount={setAmount} startDate={startDate} setStartDate={setStartDate} endDate={endDate} setEndDate={setEndDate} createOpportunity={() => createOpportunity.mutate()} onSaved={refresh} />}
...
function ContactDetailPanel({ value, api, corpId, canEdit, tagId, setTagId, bindTag, poolReason, setPoolReason, release, amount, setAmount, startDate, setStartDate, endDate, setEndDate, createOpportunity, onSaved }: { value: ContactDetail; api: ContactApi; corpId: number; canEdit: boolean; tagId: string; setTagId(value: string): void; bindTag(): void; poolReason: string; setPoolReason(value: string): void; release(): void; amount: string; setAmount(value: string): void; startDate: string; setStartDate(value: string): void; endDate: string; setEndDate(value: string): void; createOpportunity(): void; onSaved(): void }) {
