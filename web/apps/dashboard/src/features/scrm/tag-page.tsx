...
export function TagPage({ api }: { api: Pick<ScrmApi, 'listTags' | 'createTag' | 'bindTags'> }) {
  ...
  return <section><h1>客户标签</h1><label>新标签<input aria-label="新标签" value={name} onChange={(event) => setName(event.target.value)} /></label><button type="button" disabled={!name.trim()} onClick={() => create.mutate()}>创建标签</button><ul>{query.data.items.map((tag) => <li key={tag.id}>{tag.name}<button type="button" onClick={() => bind.mutate(tag.id)}>绑定到联系人</button></li>)}</ul></section>;
}
