...
export function createLeadApi(client: Client): LeadApi {
  return {
    ...
    async create(input) { return client.request('/scrm/leads', jsonRequest(input)) as Promise<Lead>; },
    async findDuplicates(input) {
      ...
      return client.request(`/scrm/leads/duplicates?${query.toString()}`) as Promise<{ items: Lead[] }>;
    },
    async assign(input) { return client.request('/scrm/leads/assignments', jsonRequest(input)) as Promise<{ results: LeadMutationResult[] }>; },
    async transition(input) { return client.request('/scrm/leads/transition', jsonRequest(input)) as Promise<Lead>; },
  };
}
