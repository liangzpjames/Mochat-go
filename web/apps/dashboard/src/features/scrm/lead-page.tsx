...
export function LeadPage({ api }: { api: LeadApi }) {
  ...
  return (
    <section className="scrm-lead-page">
      <header><h1>线索池</h1><p>管理企业微信来源的客户线索，并在确认后进入联系人生命周期。</p></header>
      {canAdd && <div className="scrm-lead-toolbar">
      ...
      <table><thead><tr><th>名称</th><th>业务标识</th><th>来源</th><th>状态</th><th>版本</th></tr></thead><tbody>{leads.data.items.map((lead) => <tr key={lead.id}><td>{lead.name}</td><td>{lead.businessKey}</td><td>{lead.source}</td><td>{lead.status}</td><td>{lead.version}</td></tr>)}</tbody></table>
    </section>
  );
}
