...
export function LeadPage({ api }: { api: LeadApi }) {
  ...
  const hasLegacyActionContract = access.allowedActions.size > 0;
  const canAdd = !hasLegacyActionContract || access.allowedActions.has('/customer/clue/default@add'); const canAssign = !hasLegacyActionContract || access.allowedActions.has('/customer/clue/default@assign'); const canEdit = !hasLegacyActionContract || access.allowedActions.has('/customer/clue/default@edit');
  ...
  return <section className="scrm-lead-page">
    ...
    <div className="dashboard-data-card scrm-list-card">
      ...
      {(cursor || leads.data?.nextCursor) && <DashboardCursorPagination cursor={cursor} nextCursor={leads.data?.nextCursor} onCursorChange={(nextCursor) => setSearchParams(leadSearch(searchParams, applied, nextCursor))} />}
    </div>
  </section>;
}
