...
export function LeadPage({ api }: { api: LeadApi }) {
  ...
  return <section className="scrm-lead-page">
    <header className="dashboard-page-header"><div><h1>线索池</h1><p>统一筛选、分配和转化客户线索，所有操作均保留企业范围与版本校验。</p></div></header>
    <div className="dashboard-stat-grid">
      <article><span>当前结果</span><strong>{leads.data?.items.length ?? 0}</strong></article><article><span>待确认</span><strong>{leads.data?.items.filter((item) => item.status === 'new').length ?? 0}</strong></article><article><span>已确认</span><strong>{leads.data?.items.filter((item) => item.status === 'qualified').length ?? 0}</strong></article><article><span>已选择</span><strong>{Object.keys(selected).length}</strong></article>
    </div>
