...
export function KnowledgeBasePage({ api }: { api: AISettingsApi }) {
  ...
  return (
    <Phase35PageShell title="AI 知识库" description="维护企业知识库：名称、说明、文档数量与启停状态" actions={<button ref={triggerRef} type="button" onClick={openCreate}>新建知识库</button>}>
      <div className="phase35-page">
                  ...
                  {items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        ...
                        <ConfirmAction title={`确认删除知识库“${item.name}”？`} onConfirm={() => remove.mutate(item.id)}><button type="button">{`删除 ${item.name}`}</button></ConfirmAction>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <DashboardDialog open={creating || editing !== null} title={editing ? '编辑知识库' : '新建知识库'} triggerRef={triggerRef} confirmDisabled={!valid} confirmLoading={save.isPending} onCancel={closeEditor} onConfirm={() => save.mutate()}>
