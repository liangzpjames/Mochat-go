...
export function MaterialManagementPage({ api }: { api: BusinessWorkbenchApi }) {
  ...
  const createGroup = async () => {
    ...
    try { await api.write('/mediumGroup/store', { name }, 'POST'); await groupQuery.refetch(); setGroupComposerOpen(false); setGroupName(''); }
    catch (error) { setGroupError(error instanceof Error ? error.message : '创建分组失败'); }
    finally { setGroupSaving(false); }
  };
  return <section className="phase34-page phase34-material-page"><header className="phase34-page-header phase34-material-header"><div><p className="phase34-eyebrow">营销工具 · 素材底座</p><h1>素材管理</h1><p>统一沉淀企业内容，在朋友圈、群发、活码和聊天侧边栏中安全复用。</p></div><div className="phase34-header-actions"><span className="phase34-provider-badge">真实素材数据</span><button type="button" aria-label="添加素材" onClick={() => { setActionError(''); setComposerOpen(true); }}>＋ 添加素材</button><button type="button" className="phase34-secondary-button" onClick={() => void materialQuery.refetch()}>刷新</button></div></header><div className="phase34-material-scopes" role="tablist" aria-label="素材作用域">{scopes.map((item) => <button key={item.id} type="button" role="tab" aria-controls="material-scope-panel" aria-selected={scope === item.id} className={scope === item.id ? 'active' : ''} onClick={() => { setScope(item.id); setSelected(new Set()); }}><strong>{item.label}</strong><span>{item.description}</span></button>)}</div><div id="material-scope-panel" role="tabpanel" className="phase34-material-workbench"><aside className="phase34-material-groups"><div><h2>素材分组</h2><button type="button" aria-label="创建分组" onClick={() => { setGroupName(''); setGroupError(''); setGroupComposerOpen(true); }}>＋</button></div>{groupQuery.isPending ? <p>加载分组…</p> : materialGroups.map((group) => <button type="button" key={group.id} className={groupId === group.id ? 'active' : ''} onClick={() => setGroupId(group.id)}><span>{group.name}</span></button>)}</aside><div><div className="phase34-material-filter"><label>搜索素材<input aria-label="搜索素材" placeholder="搜索名称或正文" value={draftKeyword} onChange={(event) => setDraftKeyword(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') applyKeyword(); }} /></label><label>素材类型<select aria-label="素材类型" value={type} onChange={(event) => setType(Number(event.target.value))}><option value={0}>全部类型</option><option value={1}>文本</option><option value={2}>图片</option><option value={6}>文件</option></select></label><button type="button" onClick={applyKeyword}>查询</button><button type="button" disabled={!draftKeyword && !keyword && !type} onClick={() => { setDraftKeyword(''); setKeyword(''); setType(0); }}>重置</button></div>{selected.size > 0 && <div className="phase34-material-batch"><strong>已选择 {selected.size} 项</strong><select aria-label="移动到分组" value={moveGroupId} onChange={(event) => setMoveGroupId(Number(event.target.value))}>{materialGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select><button type="button" onClick={() => void batchMove()}>批量移动</button><ConfirmAction title={`确认删除已选择的 ${selected.size} 条素材？`} description="删除后无法恢复，且正在被业务引用的素材会被系统阻止删除。" onConfirm={() => { void batchDelete(); }}><button type="button" className="danger">批量删除</button></ConfirmAction></div>}{actionError && <p className="phase34-material-error" role="alert">{actionError}</p>}{materialQuery.isPending ? <PageState state="loading" /> : materialQuery.isError ? <PageState state={pageStateForError(materialQuery.error)} onRetry={() => void materialQuery.refetch()} /> : rows.length === 0 ? <PageState state="empty" title="暂无素材" description="添加第一条可复用素材，建立企业内容底座。" /> : <div className="phase34-material-grid">{rows.map((item) => { const title = materialTitle(item); return <article key={item.id}><div className="phase34-material-card-top"><input type="checkbox" aria-label={`选择${title}`} checked={selected.has(item.id)} onChange={() => toggle(item.id)} /><span>{item.type}</span></div><h3>{title}</h3><p>{materialPreview(item)}</p><dl><div><dt>分组</dt><dd>{item.mediumGroupName || '未分组'}</dd></div><div><dt>创建人</dt><dd>{item.userName || '--'}</dd></div></dl><footer><span className={`phase34-material-status ${item.status}`}>{item.status === 'available' ? '可用' : item.status}</span><time>{item.createdAt}</time></footer></article>; })}</div>}</div></div>{groupComposerOpen && <GroupComposer name={groupName} saving={groupSaving} error={groupError} onNameChange={setGroupName} onClose={() => { if (!groupSaving) setGroupComposerOpen(false); }} onSave={() => void createGroup()} />}{composerOpen && <MaterialComposer groups={materialGroups} departments={departmentOptions} scope={scope} saving={saving} onClose={() => setComposerOpen(false)} onSave={(values) => void createMaterial(values)} />}</section>;
}
