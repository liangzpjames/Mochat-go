    ...
    <aside className="phase34-detail" aria-label={label}>
      ...
      <div className="phase34-detail-panel">
        ...
        <form className="phase34-detail-form" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
          ...
          {target && <p className="phase34-field-hint">仅支持站内路径，短链访问和转化会记录在当前企业下。</p>}
...
export function RedirectLinkPage({ api }: { api: BusinessWorkbenchApi }) {
  ...
  return (
    <section className="phase34-page">
      <header className="phase34-page-header"><div><p className="phase34-eyebrow">营销工具 · 转化承接</p><h1>获客链接</h1><p>以可追踪链接承接外部访问，并查看访问与新增客户转化。</p></div><div className="phase34-header-actions"><span className="phase34-provider-badge">功能已就绪</span>{can('create') && <button type="button" onClick={() => { setWriteError(''); setCreateOpen(true); }}>创建获客链接</button>}{can('authorize') && <button type="button" disabled={authorizing} onClick={() => { void authorize(); }}>立即授权并使用</button>}{can('refresh') && <button type="button" disabled={query.isFetching} onClick={refresh}>刷新</button>}</div></header>
...
export function WechatCustomerServicePage({ api }: { api: BusinessWorkbenchApi }) {
  ...
  return (
    <section className="phase34-page">
      <header className="phase34-page-header"><div><p className="phase34-eyebrow">营销工具 · 转化承接</p><h1>微信客服</h1><p>管理企业微信客服账号、接待员工与接待方式。</p></div><div className="phase34-header-actions"><span className="phase34-provider-badge">功能已就绪</span>{can('create') && <button type="button" onClick={() => { setWriteError(''); setCreateOpen(true); }}>创建客服</button>}{can('sync') && <button type="button" disabled={syncing} onClick={() => { void sync(); }}>同步客服账号</button>}{can('refresh') && <button type="button" disabled={query.isFetching} onClick={refresh}>刷新</button>}</div></header>
      ...
      <div className="dashboard-data-card phase34-results-card"><div className="dashboard-card-heading"><div><h2>微信客服列表</h2><p>当前企业：{access.corp.name}，这里展示已保存的客服记录和同步状态。</p></div><span>{rows.length} 条</span></div>{writeError && !createOpen && <p role="alert" className="phase34-inline-error">{writeError}</p>}{query.isPending ? <PageState state="loading" /> : query.isError ? <PageState state={pageStateForError(query.error)} {...(can('refresh') ? { onRetry: refresh } : {})} /> : rows.length === 0 ? <PageState state="empty" title="暂无客服账号" description="创建客服记录后，再通过同步操作获取外部客服账号。" /> : <div className="dashboard-table-scroll phase34-table-scroll"><table className="phase34-table"><thead><tr><th>客服名称</th><th>客服账号</th><th>接待员工</th><th>接待方式</th><th>状态</th><th>创建人 / 创建日期</th></tr></thead><tbody>{rows.map((row, index) => <tr key={rowKey(row, index)}><td><strong>{display(row.name)}</strong></td><td>{display(row.account)}</td><td>{display(row.employeeIds)}</td><td>{display(row.receiveMode)}</td><td>{conversionStatus(row.status)}</td><td>{display(row.creatorName)} / {display(row.createdAt)}</td></tr>)}</tbody></table></div>}</div>
    ...
    <aside className="phase34-detail" aria-label="一键加群模板">
      ...
      <div className="phase34-detail-panel">
        ...
        <form className="phase34-detail-form" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
          ...
          <p id="group-template-employee-hint" className="phase34-field-hint">来源：当前企业已同步员工的企业微信成员 ID；暂无可选员工时请先完成同步。</p>
          ...
          <p id="group-template-rooms-hint" className="phase34-field-hint">请输入 JSON 数组；每项为系统可识别的群聊配置对象，例如 {'[{"roomId": 123}]'}。</p>
          {roomsError && <p role="alert" className="phase34-inline-error">{roomsError}</p>}
          <p className="phase34-field-hint">创建后系统会生成企业微信二维码；外部操作失败时会保留错误，不会显示无效二维码。</p>
          {error && <p role="alert" className="phase34-inline-error">{error}</p>}
  ...
  return (
    <section className="phase34-page">
      <header className="phase34-page-header">
        <div><p className="phase34-eyebrow">营销工具 · 转化承接</p><h1>一键加群</h1><p>使用自动拉群功能管理加群模板，查看关联群聊和当日新增。</p></div>
      ...
      <div className="dashboard-data-card phase34-results-card">
        <div className="dashboard-card-heading"><div><h2>加群模板列表</h2><p>当前企业：{access.corp.name}，展示系统中保存的真实模板记录。</p></div><span>{rows.length} 条</span></div>
