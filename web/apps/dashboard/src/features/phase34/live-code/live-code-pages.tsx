...
function LiveCodeCreateDialog({
  ...
  const [roomBaseID, setRoomBaseID] = useState('1');
  ...
  useEffect(() => {
    ...
    const validIDs = new Set((roomsQuery.data ?? []).map((room) => room.id));
  ...
  const parsedRoomBaseID = Number(roomBaseID);
  const autoCreateValid = !autoCreateRoom || (roomBaseName.trim() !== '' && Number.isInteger(parsedRoomBaseID) && parsedRoomBaseID > 0);
  const valid = name.trim() !== '' && (kind === 'channel' ? selectedEmployeesValid : selectedRoomsValid && autoCreateValid);
  ...
  const submit = () => {
    ...
    if (kind === 'channel') {
      ...
      onSave({
        ...
        autoCreateRoom,
        roomBaseName: autoCreateRoom ? roomBaseName.trim() : '',
        roomBaseId: autoCreateRoom ? parsedRoomBaseID : 1,
      });
    }
  };
  return (
        ...
        <div className="phase34-live-code-drawer-body">
          <form className="phase34-detail-form" onSubmit={(event) => { event.preventDefault(); submit(); }}>
            ...
            {kind === 'group' && <>
              ...
              <section className="phase34-live-code-auto-room" aria-label="入群设置">
                <div><strong>入群设置</strong><small>已选群聊满员后，可由企业微信按序创建新群。</small></div>
                <label className="phase34-live-code-switch"><input type="checkbox" aria-label="群满后自动创建新群" checked={autoCreateRoom} onChange={(event) => setAutoCreateRoom(event.target.checked)} /><span aria-hidden="true" />群人数满 200 人后自动创建新群</label>
                {autoCreateRoom && <div className="phase34-live-code-auto-room-fields"><label>新群名称 <b>*</b><input aria-label="新群名称" value={roomBaseName} maxLength={30} onChange={(event) => setRoomBaseName(event.target.value)} placeholder="例如：售后服务群" /></label><label>起始序号 <b>*</b><input aria-label="起始序号" type="number" min="1" step="1" value={roomBaseID} onChange={(event) => setRoomBaseID(event.target.value)} /></label></div>}
              </section>
            </>}
            {error && <p className="phase34-inline-error" role="alert">{error}</p>}
          </form>
        </div>
        <footer className="phase34-live-code-drawer-footer"><span>带 * 为必填项</span><div><button type="button" className="phase34-secondary-button" disabled={saving} onClick={onCancel}>取消</button><button type="button" disabled={saving || !valid} onClick={submit}>{saving ? '生成中…' : kind === 'channel' ? '保存渠道活码' : '保存并生成二维码'}</button></div></footer>
      </div>
        ...
        <section className="phase34-live-code-picker" role="dialog" aria-label="选择群聊" aria-modal="true">
          ...
          <div className="phase34-live-code-picker-grid">
            <section aria-label="可选群聊"><label>搜索群聊<input aria-label="搜索群聊" value={roomSearch} onChange={(event) => setRoomSearch(event.target.value)} placeholder="输入群聊名称" /></label><div className="phase34-live-code-room-options">{visibleRooms.map((room) => {
              ...
              const capacityText = Number.isInteger(room.roomMax) && room.roomMax > 0 ? `当前 ${room.currentNum} 人 · 容量 ${room.roomMax} 人` : '人数与容量待同步';
              return <button key={room.id} type="button" aria-label={`${selected ? '取消选择' : '选择'}群聊 ${room.name}`} aria-pressed={selected} disabled={limitReached} onClick={() => toggleRoomDraft(room)}><span className="phase34-live-code-room-mark" aria-hidden="true">{selected ? '✓' : ''}</span><span><strong>{room.name}</strong><small>{capacityText}</small></span></button>;
...
function DetailDrawer({ kind, row, onClose }: { kind: LiveCodeKind; row: LiveCodeRecord; onClose: () => void }) {
  ...
  return (
    <aside className="live-code-detail" aria-label={`${kind === 'channel' ? '渠道活码' : '群活码'}详情`}>
      ...
      <div className="phase34-detail-panel" role="dialog" aria-modal="true">
        <header className="phase34-live-code-drawer-header">
          <div className="phase34-live-code-drawer-title"><span className="phase34-live-code-drawer-icon" aria-hidden="true">码</span><div><p className="phase34-eyebrow">营销工具 · {isChannel ? '渠道获客' : '社群获客'}</p><h2>{name}</h2><p>{isChannel ? '查看二维码、使用成员和新增好友数据。' : '查看企业微信二维码和关联群聊配置。'}</p></div></div>
        ...
        <div className="phase34-live-code-drawer-body">
          ...
          <section className="phase34-live-code-section"><h3>基础信息</h3><dl>
            ...
            <div><dt>{isChannel ? '创建人' : '活码类型'}</dt><dd>{isChannel ? detail('—', 'creator', 'creatorName') : detail('contact_way', 'providerKind') === 'join_way' ? '直接进群' : '旧版活码'}</dd></div>
            {isChannel && <div><dt>使用成员</dt><dd>{employeesText(row)}</dd></div>}
...
function GroupCodeResult({ name, url, onClose }: { name: string; url: string; onClose: () => void }) {
  return <aside className="phase34-detail phase34-live-code-result" aria-label="群活码生成结果"><div className="phase34-detail-backdrop" aria-hidden="true" onClick={onClose} /><section className="phase34-live-code-result-card" role="dialog" aria-modal="true"><span className="phase34-live-code-result-mark" aria-hidden="true">✓</span><h2>群活码已生成</h2><p>二维码由企业微信返回，客户扫码后可直接进入已配置的客户群。</p><img src={url} alt={`${name}群活码二维码`} /><strong>{name}</strong><button type="button" onClick={onClose}>完成</button></section></aside>;
}
...
export function LiveCodeWorkspacePage({ api, kind }: { api: BusinessWorkbenchApi; kind: LiveCodeKind }) {
  ...
  const [groupResult, setGroupResult] = useState<{ name: string; url: string } | null>(null);
  ...
  const save = async (values: Record<string, unknown>) => {
    ...
    try {
      const result = await api.write(isChannel ? '/channelCode/store' : '/workRoomAutoPull/store', values, 'POST');
      if (!isChannel) {
        const payload = typeof result === 'object' && result !== null ? result as Record<string, unknown> : {};
        const qrcodeUrl = textOf(value(payload, 'qrcodeUrl', 'qrCodeUrl'), '');
        if (!qrcodeUrl) throw new Error('企业微信未返回可用的群活码二维码，请稍后重试');
        setGroupResult({ name: textOf(values.qrcodeName, '群活码'), url: qrcodeUrl });
      }
  ...
  return (
    <section className="live-code-page">
      <header className="live-code-header"><div><span className="live-code-kicker">营销工具 · 渠道获客</span><h1>{title}</h1><p>{isChannel ? '用清晰的归因与人员信息管理每一个获客入口。' : '选择企业微信群并生成客户可直接扫码加入的真实二维码。'}</p></div><div className="live-code-header-actions"><span className="live-code-connection"><i />真实数据已连接</span>{can('create') ? <button type="button" onClick={() => { setWriteError(''); setCreateOpen(true); }}>新建{title}</button> : <span className="live-code-readonly">当前账号仅可查看{title}</span>}</div></header>
      ...
      {groupResult && <GroupCodeResult name={groupResult.name} url={groupResult.url} onClose={() => setGroupResult(null)} />}
    </section>
  );
}
