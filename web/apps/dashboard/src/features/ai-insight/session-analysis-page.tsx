...
export function SessionAnalysisPage({ api, navigate }: WorkspaceProps) {
  ...
  useEffect(() => {
    ...
    void Promise.all([api.sessionRecords(applied), api.sessionStatus()])
      .then(([rows, run]) => { if (active) { setPage(rows ?? { page: 1, pageSize: 20, total: 0, items: [] }); setStatus(run); } })
      .catch((reason: unknown) => { if (active) setError(reason instanceof Error ? reason.message : '加载失败'); })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [api, applied]);

  const apply = (next: SessionInsightFilters) => {
    const normalized = { ...next, page: next.page || 1 };
    setApplied(normalized); setDraft(normalized); window.history.replaceState({}, '', writeSessionFilters(normalized));
  };
  const open = (id: number) => {
    setDetailLoading(true);
    void api.sessionDetail(id).then(setDetail).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : '详情加载失败')).finally(() => setDetailLoading(false));
  };
  ...
  return <div className="ai-insight-workspace">
    ...
    <section className="ai-insight-results"><header className="ai-insight-results-header"><div><h2>分析结果</h2><p>每条记录对应一个真实归档会话，固定每页 20 条</p></div><span>{page.total} 条</span></header>{loading ? <div className="ai-insight-loading">正在读取分析结果…</div> : error ? <div className="ai-insight-error" role="alert">{error}</div> : page.items.length === 0 ? <div className="ai-insight-empty">当前筛选暂无分析结果</div> : <SessionTable page={page} onOpen={open} />}<InsightPagination page={page.page} total={page.total} onChange={(next) => apply({ ...applied, page: next })} /></section>
