...
export type SmartInsightFilters = SessionInsightFilters & { ruleVersionId?: number | undefined };
export type SessionInsightRow = { id: number; conversationKey: string; employee: Person; target: Person & { type: ConversationType }; sourceWindow: SourceWindow; status: InsightStatus; summary: string; analysisAt: string; result?: Record<string, unknown>; provider: string; model: string; promptVersion: string };
...
export type AiInsightWorkspaceApi = {
  ...
  smartRules(filters?: { status?: string; keyword?: string }): Promise<{ page: number; pageSize: 20; total: number; items: AnalysisRule[] }>;
...
function requestBody(method: string, body: unknown): RequestInit { return { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }; }
...
export function createAiInsightWorkspaceApi(client: Client): AiInsightWorkspaceApi {
  return {
    ...
    async smartRules(filters = {}) { const source = record(await client.request(`/ai-insight/smart-analysis/rules?${query(filters)}`)); const rawItems = Array.isArray(source.items) ? source.items : []; const items = rawItems.map((value) => { const item = record(value); const id = number(item.id); const name = text(item.name); if (id <= 0 || !name) throw new Error('智能分析规则接口返回了无效数据'); return { id, name, objective: text(item.objective), conversationTypes: (Array.isArray(item.conversationTypes) ? item.conversationTypes : []).map(type), targetScope: text(item.targetScope) as AnalysisRule['targetScope'], targetIds: Array.isArray(item.targetIds) ? item.targetIds.map(number) : [], lookbackDays: number(item.lookbackDays), minimumMessages: number(item.minimumMessages), status: text(item.status) as AnalysisRule['status'], currentVersion: number(item.currentVersion), createdAt: text(item.createdAt), updatedAt: text(item.updatedAt) }; }); return { page: Math.max(1, number(source.page)), pageSize: 20 as const, total: number(source.total), items }; },
    async createRule(input) { return parseRule(await client.request('/ai-insight/smart-analysis/rules', requestBody('POST', input))); },
    async updateRule(input) { return parseRule(await client.request('/ai-insight/smart-analysis/rules', requestBody('PUT', input))); },
    async setRuleStatus(id, value) { await client.request('/ai-insight/smart-analysis/rules/status', requestBody('POST', { id, status: value })); },
    async deleteRule(id) { await client.request(`/ai-insight/smart-analysis/rules?id=${encodeURIComponent(id)}`, requestBody('DELETE', {})); },
  };
}
function parseRule(value: unknown): AnalysisRule { const source = record(value); const name = text(source.name); const id = number(source.id); if (id <= 0 || !name) throw new Error('智能分析规则接口返回了无效数据'); return { id, name, objective: text(source.objective), conversationTypes: (Array.isArray(source.conversationTypes) ? source.conversationTypes : []).map(type), targetScope: text(source.targetScope) as AnalysisRule['targetScope'], targetIds: Array.isArray(source.targetIds) ? source.targetIds.map(number) : [], lookbackDays: number(source.lookbackDays), minimumMessages: number(source.minimumMessages), status: text(source.status) as AnalysisRule['status'], currentVersion: number(source.currentVersion), createdAt: text(source.createdAt), updatedAt: text(source.updatedAt) }; }
