...
function parseDerivedRow(value: unknown, view: DerivedInsightView): SessionInsightRow {
  ...
  if (parsed.status === 'succeeded') {
    if (!parsed.provider.trim() || !parsed.model.trim() || !parsed.promptVersion.trim() || !parsed.analysisAt.trim() || !Number.isFinite(Date.parse(parsed.analysisAt))) throw new Error('成功结果缺少追溯元数据');
