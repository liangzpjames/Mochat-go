...
describe('AI 洞察专用投影 URL 状态', () => {
  ...
  it('写入时清除其他 view 的陈旧专用参数并保留 0 分', () => {
    ...
    const scoreURL = state.writeDerivedFilters('employee-score', { page: 1, employeeId: 1001, customerName: '客户乙', minScore: 0, maxScore: 100 }, stale);
    expect(new URL(scoreURL).search).toBe('?employeeId=1001&customerName=%E5%AE%A2%E6%88%B7%E4%B9%99&minScore=0&maxScore=100');
    expect(state.readDerivedFilters('employee-score', scoreURL)).toEqual({ page: 1, employeeId: 1001, customerName: '客户乙', minScore: 0, maxScore: 100 });
