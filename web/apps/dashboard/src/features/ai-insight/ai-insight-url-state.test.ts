...
describe('AI 洞察专用投影 URL 状态', () => {
  it('三页只恢复 common 条件和各自专用条件', () => {
    ...
    expect(readDerivedFilters('emotion', `http://localhost/ai-insight/emotion?${common}&emotion=negative&minScore=0&maxScore=100&keyword=old`)).toEqual({
      page: 2, employeeId: 1001, customerName: '客户甲', status: 'succeeded', startDate: '2026-08-20', endDate: '2026-08-24', emotion: 'negative',
    });
    expect(readDerivedFilters('employee-score', `http://localhost/ai-insight/employee-score?${common}&emotion=positive&minScore=0&maxScore=100&keyword=old`)).toEqual({
      page: 2, employeeId: 1001, customerName: '客户甲', status: 'succeeded', startDate: '2026-08-20', endDate: '2026-08-24', minScore: 0, maxScore: 100,
    });
    expect(readDerivedFilters('communication-keyword', `http://localhost/ai-insight/communication-keyword?${common}&emotion=mixed&minScore=0&maxScore=100&keyword=50%25_%5C%E9%87%87%E8%B4%AD`)).toEqual({
      page: 2, employeeId: 1001, customerName: '客户甲', status: 'succeeded', startDate: '2026-08-20', endDate: '2026-08-24', keyword: '50%_\\采购',
    });
  });
  ...
  it('丢弃非法、越界和不属于当前 view 的参数', () => {
    ...
    expect(readDerivedFilters('emotion', invalid)).toEqual({ page: 1 });
    expect(readDerivedFilters('employee-score', 'http://localhost/ai-insight/employee-score?minScore=90&maxScore=20')).toEqual({ page: 1 });
    expect(readDerivedFilters('employee-score', 'http://localhost/ai-insight/employee-score?minScore=0&maxScore=100.5')).toEqual({ page: 1, minScore: 0 });
  });
  ...
  it('写入时清除其他 view 的陈旧专用参数并保留 0 分', () => {
    ...
    const scoreURL = writeDerivedFilters('employee-score', { page: 1, employeeId: 1001, customerName: '客户乙', minScore: 0, maxScore: 100 }, stale);
    expect(new URL(scoreURL).search).toBe('?employeeId=1001&customerName=%E5%AE%A2%E6%88%B7%E4%B9%99&minScore=0&maxScore=100');
    expect(readDerivedFilters('employee-score', scoreURL)).toEqual({ page: 1, employeeId: 1001, customerName: '客户乙', minScore: 0, maxScore: 100 });

    const keywordURL = writeDerivedFilters('communication-keyword', { page: 3, keyword: '采购' }, scoreURL);
  ...
  it('查询条件变化可回到第一页，浏览器返回的旧 URL 可完整恢复', () => {
    ...
    const queried = writeDerivedFilters('emotion', { page: 1, employeeId: 1001, emotion: 'positive' }, oldPage);
    expect(new URL(queried).searchParams.has('page')).toBe(false);
    expect(readDerivedFilters('emotion', queried)).toEqual({ page: 1, employeeId: 1001, emotion: 'positive' });
    expect(readDerivedFilters('emotion', oldPage)).toEqual({ page: 4, employeeId: 1001, emotion: 'negative' });
  });
});
