...
const sidebarCases = [
  { path: '/', title: '客户侧边栏', moduleLabel: '客户侧边栏模块待迁移', needsSession: true },
  { path: '/auth', title: '企业微信授权回调', moduleLabel: '登录失败', needsSession: false },
  { path: '/codeAuth', title: '企业微信扫码授权', moduleLabel: '企业微信扫码授权模块待迁移', needsSession: false },
  { path: '/contact', title: '客户资料', moduleLabel: '浏览器验收客户', needsSession: true, query: '?wxExternalUserid=external-user-1&agentId=7' },
  { path: '/contact/editDetail', title: '编辑客户资料', moduleLabel: '编辑客户资料模块待迁移', needsSession: true },
  { path: '/contact/remark', title: '客户备注', moduleLabel: '客户备注模块待迁移', needsSession: true },
  { path: '/contact/settingTag', title: '设置客户标签', moduleLabel: '设置客户标签模块待迁移', needsSession: true },
  { path: '/contactBatchAdd', title: '批量加好友', moduleLabel: '批量加好友模块待迁移', needsSession: true },
  { path: '/contactSop', title: '个人客户 SOP', moduleLabel: '个人客户 SOP模块待迁移', needsSession: true },
  { path: '/login', title: '侧边栏登录', moduleLabel: '继续授权', needsSession: false, query: '?agentId=7&target=%2Fcontact' },
  { path: '/medium', title: '素材库', moduleLabel: '素材库模块待迁移', needsSession: true },
  { path: '/roomSop', title: '客户群 SOP', moduleLabel: '客户群 SOP模块待迁移', needsSession: true },
] as const;

const operationCases = [
  { path: '/', title: '营销活动中心', moduleLabel: '营销活动中心模块待迁移' },
  { path: '/explain', title: '活动说明', moduleLabel: '活动说明模块待迁移' },
  { path: '/lottery', title: '抽奖活动', moduleLabel: '抽奖活动模块待迁移' },
  { path: '/roomClockIn', title: '群打卡', moduleLabel: '群打卡模块待迁移' },
  { path: '/roomFission', title: '群裂变活动', moduleLabel: '群裂变活动模块待迁移' },
  { path: '/fissionSpeed', title: '群裂变进度', moduleLabel: '群裂变进度模块待迁移' },
  { path: '/roomInfinitePull', title: '无限拉群', moduleLabel: '无限拉群模块待迁移' },
  { path: '/shopCode', title: '门店活码', moduleLabel: '门店活码模块待迁移' },
  { path: '/workFission', title: '任务宝活动', moduleLabel: '任务 1：邀请 3 位好友', query: '?union_id=union-browser-1&fission_id=17' },
  { path: '/speed', title: '任务宝进度', moduleLabel: '任务宝进度模块待迁移', query: '?union_id=union-browser-1&fission_id=17' },
] as const;
...
async function installRawGoFixtures(page: Page): Promise<BrowserAudit> {
  ...
  await page.route('**/sidebar/workContact/detail?*', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        requestId: 'sidebar-e2e-request-1',
      }),
    });
  });
  await page.route('**/operation/workFission/taskData?*', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        requestId: 'operation-e2e-request-1',
      }),
    });
  });
  return audit;
...
async function assertVisiblePage(page: Page, title: string, moduleLabel: string): Promise<void> {
  ...
  for (let index = 0; index < await controls.count(); index += 1) {
    ...
    expect(box?.height ?? 0, `${title} primary or retry control is shorter than 44px`).toBeGreaterThanOrEqual(44);
  }
}
  ...
  test.describe(viewport.name, () => {
    ...
    for (const routeCase of sidebarCases) {
      test(`Sidebar ${routeCase.path} renders its route title and module`, async ({ page }) => {
        ...
        await assertVisiblePage(page, routeCase.title, routeCase.moduleLabel);
        expect(audit.contactRequests).toHaveLength(routeCase.path === '/contact' ? 1 : 0);
        assertCleanAudit(audit, `${viewport.name} Sidebar ${routeCase.path}`);
      });
    }
    ...
    for (const routeCase of operationCases) {
      test(`Operation ${routeCase.path} renders its route title and module`, async ({ page }) => {
        ...
        assertCleanAudit(audit, `${viewport.name} Operation ${routeCase.path}`);
      });
    }
    ...
    test('Sidebar unknown path shows 404 and does not render home content', async ({ page }) => {
      ...
      await assertVisiblePage(page, '未找到页面', '页面不存在');
      assertCleanAudit(audit, `${viewport.name} Sidebar unknown`);
    });
    ...
    test('Operation unknown path shows 404 and does not render home content', async ({ page }) => {
      ...
      await assertVisiblePage(page, '未找到活动页面', '页面不存在');
      assertCleanAudit(audit, `${viewport.name} Operation unknown`);
    });
  });
}
