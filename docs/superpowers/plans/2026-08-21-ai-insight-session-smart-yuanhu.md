...
- [ ] **Step 4: 运行 GREEN、lint 和 typecheck**
...
- [ ] **Step 4: 运行 GREEN、typecheck 和布局测试**
...
- [ ] 只改会话分析和智能分析两个菜单；
- [ ] 两页不再渲染通用能力三卡片和大块介绍；
- [ ] 会话分析结果按真实会话、来源窗口和来源指纹落库；
- [ ] 智能分析结果关联真实不可变规则版本；
- [ ] 模型 JSON、枚举、分数、置信度和证据消息均严格校验；
- [ ] 页面打开、查询和刷新不调用模型；
- [ ] tenant/corp/员工范围覆盖列表、详情、导出和规则对象范围；
- [ ] 规则管理、导出和详情使用独立 action RBAC；
- [ ] 固定每页 20 条，筛选、标签、页码和详情 ID 可由 URL 恢复；
- [ ] 头像 fallback 使用名称首字，不出现固定“请”字；
- [ ] 证据不足显示“证据不足”，不伪造 0 分；
- [ ] 旧三个 AI 洞察页和旧 `mochat_go_ai_analysis` 兼容行为未被破坏；
- [ ] 目标测试、相关全量测试、typecheck、build 与 Go 回归通过；
- [ ] 0148 up/down、健康版本、索引和 MariaDB 方言验证完成；
- [ ] app-only Docker 交付后 MySQL、Redis 和命名卷保持健康；
- [ ] 两个分辨率的真实浏览器点击验收完成；
- [ ] 正式企微归档、正式 AI Key、数据协议和成本治理只记录在总进度，不在页面重复铺陈；
- [ ] 未覆盖、删除或误提交工作树中其他人的改动。
