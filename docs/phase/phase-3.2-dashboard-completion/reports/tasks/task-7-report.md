...
## 2026-08-02 联系人页面体验优化

- 统一页面头部、真实数据摘要、组合筛选、列表工具栏、标签徽章和状态标识，新增保留当前筛选的刷新入口。
- 联系人聚合详情改为右侧可访问抽屉，支持 Escape 关闭、焦点进入和恢复；分配、标签、公海、商机与跟进仍使用既有真实接口。
- 未增加批量删除、批量转移、好友关系编辑等后端不存在的能力。
- 浏览器确认 SCRM 分组展开、联系人选中，以及新版头部、摘要、筛选、列表和真实错误状态。
- 自动化证据：联系人 6 项测试通过；Dashboard 全量 56 个文件、377 项测试通过；类型检查与生产构建通过。
