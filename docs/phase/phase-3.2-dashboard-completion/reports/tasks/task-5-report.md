...
## 2026-08-02 敏感词页面体验优化

- 本轮仅优化 `/ai-insight/v2/sensitive-word`，未修改其他风险预警页面。
- 命中记录增加真实结果摘要、刷新、保留筛选条件的分页和结构化详情抽屉；移除直接展示原始 JSON 的详情方式。
- 配置区重新组织词组与词条卡片，保留新增、搜索、改名、启停、移动、删除及权限控制。
- 未实现后端不存在的风险等级、AI 摘要、批量审计和导出，不伪造数据。
- 自动化证据：敏感词页面 6 项测试通过；Dashboard 全量 56 个文件、373 项测试通过；类型检查与生产构建通过。
- 浏览器证据：部署后风险预警分组保持展开、敏感词选中；记录与配置空状态、筛选、摘要和工具栏可见。
