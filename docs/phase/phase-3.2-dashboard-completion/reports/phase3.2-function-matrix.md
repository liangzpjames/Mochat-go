...
本矩阵仅覆盖 Phase 3.2 的八个指定页面。每行是一项可独立验收的参考功能；`已对应` 仅表示已建立目标实现映射，不代表该项已完成。`待` 状态会被完成门禁视为未闭合，不能支持 `e2e-passed`。

| page | referenceFeature | decision | mochatEntry | frontend | api | permission | persistence | tests | evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
