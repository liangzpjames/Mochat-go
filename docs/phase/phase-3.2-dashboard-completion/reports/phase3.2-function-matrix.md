...
| /chat/v2-all | 会话列表检索与详情 | 已对应 | - | - | - | 全局消息 | web/apps/dashboard/src/features/conversation-global/conversation-global-page.tsx | internal/dashboard/auto_tag_dashboard.go（关键词、对象类型、员工、半开日期区间、分页与真实消息 ID 详情） | internal/dashboard/auto_tag_dashboard.go（认证 tenant/corp、RBAC 数据范围交集、列表与详情同范围、403/40301/404） | internal/store/auto_tag.go（十张 mc_work_message_* 归档表参数化 UNION、企业范围与员工范围） | internal/dashboard/auto_tag_dashboard_test.go；internal/store/work_message_test.go（真实 MariaDB fixture 与 EXPLAIN）；web/apps/dashboard/src/features/conversation-global/*.test.ts* | 待 Task 11：浏览器验收证据（本任务不伪造） |
