...
| /chat/v2-all | 会话列表检索与详情 | 已对应 | - | - | - | 全局消息 | web/apps/dashboard/src/features/conversation-global/conversation-global-page.tsx | internal/dashboard/auto_tag_dashboard.go（关键词、对象类型、员工、半开日期区间、分页与真实消息 ID 详情） | internal/dashboard/auto_tag_dashboard.go（认证 tenant/corp、列表与详情统一 RBAC key、数据范围交集、403/40301/404） | internal/store/auto_tag.go（十张 mc_work_message_* 归档表参数化 UNION、企业/员工/类型/半开时间谓词下推、详情按 ID 分片定位）；deploy/standalone/migrations/0106_work_message_global_search_indexes.up.sql | internal/dashboard/auto_tag_dashboard_test.go；internal/store/work_message_test.go（真实 MariaDB fixture，覆盖权限、筛选、分页以及十表列表 `range/global_search`、详情 `ref/corp_msgid` EXPLAIN）；web/apps/dashboard/src/features/conversation-global/*.test.ts*；Dashboard 全量 55 files/332 tests | 待 Task 11：浏览器验收证据（本任务不伪造） |
