...
## 2026-08-02 线索池页面体验优化

- 统一页面头部、真实数据摘要、筛选、创建、批量分配和列表工具栏，新增保留当前筛选的刷新入口。
- 保留真实游标分页、重复检查、状态流转、部分失败和权限控制；未增加渠道对接、删除或导出等不存在的能力。
- 浏览器确认 SCRM 分组展开、线索池选中，以及新版头部、摘要、筛选和列表层级。
- 自动化证据：线索池 8 项测试通过；Dashboard 全量 56 个文件、377 项测试通过；类型检查与生产构建通过。
