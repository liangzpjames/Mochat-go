...
本轮已将 Phase 3.3 剩余菜单全部从占位页接入 native 页面：会话运营四页和风险预警六页均已注册、测试并完成 Docker 构建验证。已有后端契约的离职员工、客户继承和客户流失页面读取真实接口；文件录音、拒绝存档以及其余风险页面在当前环境显示 provider 未接入状态，不伪造业务数据、媒体或下载链接。

> 阶段收尾说明：`/chat/file-audio` 文件录音因仍缺少可验证的音频文件存储与读取 Provider，作为 Phase 3.3 唯一明确未完成菜单保留；本次合入主线不把该页面标记为完成。拒绝存档及风险预警页面的 Provider 已在后续批次补齐。
