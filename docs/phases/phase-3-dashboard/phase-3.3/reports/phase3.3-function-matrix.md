...
| `/chat/file-audio` | 会话 | 媒体筛选→预览/下载→会话 | 会话媒体存储 | 媒体读取+二次校验 | 媒体索引、下载审计 | 高 | Phase 3.3 未完成（缺 Provider）；**Phase 3 Final 已解锁**（本地音频存储 Provider，上传/播放/删除闭环） |
...
- provider 未接入但已具备明确状态页：`/chat/file-audio`（Phase 3 Final 已接入本地存储 Provider 并闭环）；
