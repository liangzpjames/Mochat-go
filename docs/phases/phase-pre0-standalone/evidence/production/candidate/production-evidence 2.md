...
- 通过 `acceptance suite coverage`：acceptance suite coverage audit passed: smoke_scripts=72 referenced=72
- 通过 `manifest route smoke coverage`：manifest route smoke coverage audit passed: manifest_routes=224 directly_covered=224 smoke_scripts=72
- 通过 `functional module matrix`：functional module matrix audit passed: modules=28 manifest_unique_paths=213/213 manifest_route_entries=224 runtime_unique_paths=384/384 runtime_route_entries=415
...
- 缺失 `持续运行稳定性记录`：设置 `MOCHAT_EVIDENCE_STABILITY` 指向证据文件。目标部署环境持续运行记录；可指向 `soak.ndjson` 或阶段报告。
