...
> 当前阶段：Phase 7 专用 callback、SDK bridge 已部署，真实单聊/群聊拉取与存档事件通知已验证；2026-08-26 23:21 起主动定时拉取已停用，主库落库、完整通讯录/客户数据、Sidebar 应用授权与三端完整角色验收仍受外部条件阻塞
...
## 无域名企微能力验证、应用模式调研与主动拉取停用（2026-08-26）

### 真实测试结论

| 能力 | 真实结果 | 当前边界 |
| --- | --- | --- |
| 会话存档主动拉取 | 官方 Finance SDK 已完成 `seq=0→6`，取得真实单聊与群聊文本/图片类型，空页复拉与重启续拉无重复 | 隔离 Demo JSONL，不是主库消息；图片媒体字节尚未通过 `GetMediaData` 下载 |
| 会话存档事件通知 | 企业微信真实加密 GET 校验和 POST 通知均已到达服务器并返回 200 | 主库 `corp=4` 未启用完整归档凭据，事件触发同步按设计跳过 |
| 通讯录同步助手 | 可信 IP 生效；`department/simplelist` 返回 2 个部门标识，`user/list_id` 返回 12 条成员-部门关系、11 个唯一成员标识 | `department/list`、`user/get` 返回 `48009`，无法取得姓名、手机号、头像和职位，不能写成完整员工 |
| MoChat 员工同步 | 本地 Go 员工同步/凭据/绑定相关测试通过；Dashboard 企业设置与员工模块 5 个测试文件、58 项测试通过 | 生产适配器仍调用通讯录助手禁止的 `department/list`、`user/list`；企业绑定仍为待验证，主库部门/员工均为 0 |

### 调研结论与推荐架构

- 普通自建应用适合单企业，完整员工资料、企业微信登录、JSSDK 和应用消息依赖应用授权范围及可信域名/回调配置；只有服务器 IP 不能替代企业微信要求的合规域名。
- 第三方标准应用适合 MoChat 多租户 SaaS：由企业微信服务商创建统一应用，各租户安装授权，服务商按租户保存永久授权码并换取企业访问凭证。
- 自建应用代开发适合定制或私有化客户：企业扫码后创建归属于客户企业、由服务商代开发的网页应用。它把应用主页和回调托管责任转移给服务商，并没有取消服务商的认证、HTTPS 域名、回调和审核要求。
- 因此第三方/代开发不是“无域名绕过方案”。当前单企业最快闭环是准备与企业主体一致或有关联关系的备案域名并配置自建应用；正式 MoChat SaaS 则应申请服务商并以第三方标准应用为主、代开发为辅。
- MoChat 凭据模型必须拆分为：通讯录同步助手、完整员工读取应用、客户联系、Sidebar/Agent、会话存档 RSA/Secret、各类回调 Token/AES Key。能力验证也应拆分为 `ID_ONLY`、`FULL`、`BLOCKED`，不得再用一个应用 Secret 或一个总开关代表全部企微能力。

### 无域名条件下的最大化验收边界

1. 可真实验证会话存档单聊/群聊、消息类型、事件通知、游标、幂等与重启续拉；媒体文件下载、主库落库和 Dashboard 回读仍需单独闭环。
2. 可周期性读取部门/员工标识并比较脱敏摘要，验证组织结构是否发生变化；不得用空姓名或占位值冒充完整员工同步。
3. 可完整验收 SaaS Admin、Dashboard 的本地账号、租户/RBAC、页面/API、错误反馈、响应式和重启状态；这类验收不依赖企微域名，但需要相应真实角色登录态。
4. 可验收 Sidebar 12 路由、390×844 布局、未授权/空态/失败态；真实员工 OAuth、JSSDK 和客户上下文必须单列为外部条件阻塞。
5. 客户详情、客户群资料、群成员和客户联系回调需要独立客户联系应用权限；会话存档中的 `roomid`、发送者和接收者只能证明消息关系，不能替代群资料接口。

### 主动定时拉取停用记录

- 根因核对：`wecom-archive-demo` 没有内部 ticker 或服务器 cron，只在服务器本机显式调用受保护的 `POST /admin/pull` 时主动访问企业微信；此前每 15 秒日志来自主应用归档同步 cron。
- 停用前主应用配置为 `MOCHAT_GO_ENABLE_WORK_MESSAGE_ARCHIVE_SYNC_CRON=1`、间隔 15 秒，但因主库 `corps=0`，每轮均为 `fetched=0`，未真正调用企业微信拉取接口。
- 2026-08-26 23:21（CST）已将该开关改为 `0`，只使用现有本地镜像重建 app；没有在服务器编译，没有重建 MySQL、Redis 或 Demo，也没有删除卷、数据库、密钥或证据。
- app 重建后 `/healthz`、`/readyz`、Demo `/healthz` 均为 200；观察 35 秒（超过两个原调度周期）未再出现 `workMessageArchive sync cron` 日志。Demo 保持运行以接收回调，但不会主动拉取。
- 回滚点：`/opt/mochat-go/backups/disable-archive-cron-20260826T232159CST`，目录中的环境文件和容器状态校验通过。恢复时复制 `env.local.before` 覆盖当前 `.env.local` 后，仅用 `docker compose ... up -d --no-build --no-deps --force-recreate app` 重建 app；不要触碰数据库和命名卷。
