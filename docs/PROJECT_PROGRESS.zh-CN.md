...
本轮只实施 `/ai-insight/v2/timeout` 和 `/ai-insight/v2/customer-loss` 两个 AI 洞察风险预警菜单。已完成统一紧凑工作台、固定中文字段表格、筛选/分页/详情抽屉、规则与高级设置入口、确认操作和 URL 状态恢复；页面不再展示能力缺口说明卡。前期客户会话菜单的改动与本专项无关，不计入本专项交付。
