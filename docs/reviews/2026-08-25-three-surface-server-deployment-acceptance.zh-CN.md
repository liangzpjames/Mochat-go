...
- **部署结果：成功。** 服务器应用已从旧镜像切换为 `e79456cab0c20540ebe5a2174b891f69c05ec341` 对应产物；应用、MySQL、Redis 均健康，`/healthz`、`/readyz`、SaaS Admin、Dashboard、Sidebar 和 Operation 入口在部署后及应用容器重启后均返回 200。
...
- 部署阻断修复提交：`e79456cab0c20540ebe5a2174b891f69c05ec341`，提交说明为 `fix: preserve deployed migration checksum compatibility`。
- 该分支当前比 `origin/main` ahead 1，未推送、未合并；原主工作树和其他已有 worktree 的未提交/未跟踪内容均未修改。
...
2. 先新增失败测试复现历史 checksum 不被接受，再只为 `0106` 增加已验证的历史 checksum 别名；未修改迁移 SQL，未更新数据库 ledger，未放宽其他 checksum 校验。
3. 同时修正 4 个过期的 Dashboard E2E 断言/夹具，并把 Sidebar 测试名称明确改为 fixture-backed，避免把夹具称为真实接口。
...
- 修复后本地应用容器镜像：`sha256:f626c7279b52471bc74fd132fa0cac9b36a7a003381b403b21c7dc16f1d3d09d`，状态 healthy。
- 本地 `/healthz`、`/readyz`、SaaS Admin、Dashboard、Sidebar（18091）、Operation（18092）均为 200；本地迁移到 `0165` 且无 mismatch。
...
发布提交：`e79456cab0c20540ebe5a2174b891f69c05ec341`。

- 本地镜像 ID / 服务器镜像 ID：`sha256:f626c7279b52471bc74fd132fa0cac9b36a7a003381b403b21c7dc16f1d3d09d`。
- 镜像 tar SHA-256：`cc39caa53cbd05ebf2fa40a19e4342421f9741d87f3d8254b45f7db20118c7a4`。
- Git 源码归档 SHA-256：`02ea9323ece0bad2e6a18fbc4879b1b2ddc5073c28871feb2b4b1b2dce7b22f0`。
- source fingerprint：`6dbb5651b196575160ce8dccf915d58dfafb05ae81c163534728be1f1c11c7dd`。
- 服务器 release 目录：`/opt/mochat-go/releases/e79456cab0c2-20260825t1900p0800`。
- 源码归档扫描未包含 `.env.local`、node_modules、tmp、test-results 或 playwright-report。

服务器执行类别：只读盘点、数据库转储、旧镜像打标签、上传后 SHA-256 比对、`docker load`、迁移 status/apply、最新 Compose 校验与安装、app-only `compose up -d --no-build --no-deps app`、健康/就绪/日志/端口检查、应用容器 restart 复验。

迁移说明：

- 修复前首次 apply 被 `0106` checksum 门禁安全拦截，返回非零且没有应用新迁移。
- 修复后预检为 0 mismatch，成功应用 `0140`–`0165` 共 26 条。
...
- app 镜像 ID 与本地产物一致，running + healthy，restart 后仍 healthy。
- MySQL/Redis 容器 ID、运行时长和命名卷未改变。
- `/healthz`、`/readyz`、SaaS Admin、Dashboard、Sidebar 均为 200。
...
- `D:\workspace\mochat-go\output\deploy-20260825-e79456cab0c2`
- `D:\workspace\mochat-go\output\deploy-20260825-e79456cab0c2\browser\server-saas-login.png`
- `D:\workspace\mochat-go\output\deploy-20260825-e79456cab0c2\browser\server-dashboard-login-390x844.png`
- `D:\workspace\mochat-go\output\deploy-20260825-e79456cab0c2\browser\server-sidebar-login-error-390x844.png`
...
- release：`/opt/mochat-go/releases/e79456cab0c2-20260825t1900p0800`
- `migration-preflight.txt`、`migration-apply.txt`、`migration-post.txt`
- `container-post-deploy.txt`、`containers-post-deploy.txt`、`volumes-post-deploy.txt`
- `SHA256SUMS`
...
3. 从回滚目录恢复旧 `docker-compose.yml`、`.env.local` 和受保护密钥文件，恢复原权限。
4. 将 `mochat-go-rollback:20260825t183621p0800` 重新标记为 `standalone-app:latest`。
5. 在明确批准数据库恢复后，用 `/opt/mochat-go/output/deploy-backups/20260825T183621+0800/mochat.sql.gz` 恢复原数据库；不要执行 volume delete。
6. 用旧 Compose app-only 启动，复验迁移状态、`/healthz`、`/readyz`、日志、Nginx 和三端入口。
...
7. **中：修复提交仍在隔离分支，未推送、未合并。** 合并前应走代码审阅；否则后续从 `origin/main` 构建会重新遇到服务器历史 checksum 阻断。
