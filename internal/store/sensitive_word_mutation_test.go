...
func TestIntegrationSensitiveWordMutationPersistsScopeConflictIdempotencyAndAudit(t *testing.T) {
	...
	if err := db.QueryRow("SELECT COUNT(*) FROM mochat_go_saas_admin_operation_logs WHERE tenant_id = ? AND action LIKE 'sensitive_word.%' AND deleted_at IS NULL", tenantID).Scan(&count); err != nil || count != 2 {
