...
func TestRecordCapabilityDispatchResultRollsBackWhenEventAppendFails(t *testing.T) {
	...
	expectCapabilityLedgerBinding(mock, 7, 9)
	mock.ExpectQuery(regexp.QuoteMeta("SELECT status FROM mochat_go_wecom_capability_operation_results")).WithArgs(7, 9, 44, "external_user", "user-1").WillReturnError(sql.ErrNoRows)
