...
func TestSidebarWorkbenchSummaryBindsEmployeeAndCorpAcrossMetrics(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT employee.id.*FROM mc_work_contact_employee AS rel.*rel.employee_id = employee.id.*rel.corp_id = employee.corp_id.*mc_work_contact_tag_pivot AS pivot.*pivot.employee_id = employee.id.*FROM mc_work_room AS room.*room.owner_id = employee.id.*FROM mc_contact_sop_log AS contact_log.*JOIN mc_contact_sop AS contact_sop.*JOIN mc_work_contact AS sop_contact.*contact_log.employee = employee.wx_user_id.*FROM mc_room_sop_log AS room_log.*room_log.employee = employee.wx_user_id.*FROM mc_contact_batch_add_import AS batch.*batch.employee_id = employee.id.*FROM mc_work_employee AS employee.*JOIN mc_corp AS corp.*employee.id = \?.*employee.corp_id = \?`).
		WithArgs(7, 9).
		WillReturnRows(sqlmock.NewRows([]string{
			"id", "name", "avatar", "corp_name", "department_names",
			"customer_total", "added_today", "tagged_total", "owned_room_total",
			"contact_sop_records", "room_sop_pending", "batch_add_pending",
...
func TestSidebarContactSOPRecordsRequireLiveSOPAndContactAndSupportPagination(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	base := `(?s)FROM mc_contact_sop_log AS log.*JOIN mc_work_employee AS employee.*employee.id = \?.*employee.corp_id = \?.*employee.wx_user_id = log.employee.*JOIN mc_contact_sop AS sop.*sop.id = log.contact_sop_id.*sop.corp_id = log.corp_id.*JOIN mc_work_contact AS contact.*contact.wx_external_userid = log.contact.*contact.corp_id = log.corp_id.*contact.deleted_at IS NULL`
	mock.ExpectQuery(base).WithArgs(7, 9).WillReturnRows(sqlmock.NewRows([]string{"total"}).AddRow(21))
	mock.ExpectQuery(base+`.*ORDER BY scheduled_at DESC.*LIMIT \? OFFSET \?`).
		WithArgs(7, 9, 20, 20).
		WillReturnRows(sqlmock.NewRows([]string{"id", "kind", "title", "subject_name", "scheduled_at", "state"}).
			AddRow(55, "contactSop", "首日触达", "客户甲", "2026-08-23 09:00:00", "recorded"))

	page, err := store.SidebarTasks(context.Background(), dashboard.SidebarEmployee{ID: 7, CorpID: 9}, dashboard.SidebarTaskFilter{Kind: "contactSop", State: "recorded", Page: 2, PerPage: 20})
	if err != nil {
		t.Fatal(err)
	}
	if page.Total != 21 || page.TotalPage != 2 || len(page.Items) != 1 || page.Items[0].State != "recorded" {
		t.Fatalf("page=%+v", page)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
...
func TestSidebarBatchAddDoneRequiresEveryRowToLeavePendingStates(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	base := `(?s)FROM \(.*FROM mc_contact_batch_add_import AS batch.*batch.employee_id = \?.*batch.corp_id = \?.*batch.deleted_at IS NULL.*GROUP BY batch.record_id.*HAVING SUM\(CASE WHEN batch.status IN \(0, 1, 2\) THEN 1 ELSE 0 END\) = 0.*\) AS grouped`
	mock.ExpectQuery(base).WithArgs(7, 9).WillReturnRows(sqlmock.NewRows([]string{"total"}).AddRow(1))
	mock.ExpectQuery(base+`.*ORDER BY scheduled_at DESC.*LIMIT \? OFFSET \?`).
		WithArgs(7, 9, 10, 0).
		WillReturnRows(sqlmock.NewRows([]string{"id", "kind", "title", "subject_name", "scheduled_at", "state"}).
			AddRow(88, "batchAdd", "批量任务 #88", "3 位客户", "2026-08-23 11:00:00", "done"))

	page, err := store.SidebarTasks(context.Background(), dashboard.SidebarEmployee{ID: 7, CorpID: 9}, dashboard.SidebarTaskFilter{Kind: "batchAdd", State: "done", Page: 1, PerPage: 10})
	if err != nil {
		t.Fatal(err)
	}
	if page.Total != 1 || len(page.Items) != 1 || page.Items[0].State != "done" || page.Items[0].ID != 88 {
		t.Fatalf("page=%+v", page)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
