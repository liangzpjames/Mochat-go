...
func legacyAccessContextFromDashboard(dashboardAccess DashboardAccessContext, userID int, permissionKey string, corpID int, workEmployeeID int) (AccessContext, error) {
	...
	allowsUnboundSuperadmin := dashboardAccess.IsSuperAdmin && dashboardAccess.Scope == DataScopeTenant
	if dashboardAccess.ScopeRequired && (dashboardAccess.CorpID <= 0 || (dashboardAccess.WorkEmployeeID <= 0 && !allowsUnboundSuperadmin)) {
		return AccessContext{}, ErrPermissionDenied
	}
	access := AccessContext{
		User: User{
