...
type EmployeeApplyEvent struct {
	...
	// QueueTicket is generated by Redis while the idempotency claim and list
	// append are atomic. It is not part of the idempotency digest.
...
func (w *EmployeeApplyWorker) retryFailedDelivery(ctx context.Context, delivery EmployeeApplyDelivery, finishExecution func(string, error), credentialVersion uint64) {
	...
	finishExecution(taskrunner.StatusFailed, fmt.Errorf("SYNC_FAILED"))
	if deadLettered {
		w.recordDeadLetterState(ctx, int(delivery.Event.BindingID), credentialVersion, delivery.Event.QueueTicket)
	...
	w.recordRequeuedState(ctx, int(delivery.Event.BindingID), credentialVersion, delivery.Event.QueueTicket)
...
func (w *EmployeeApplyWorker) recordDeadLetterState(ctx context.Context, bindingID int, credentialVersion uint64, queueTicket string) {
	if credentialVersion == 0 {
		return
	}
	if err := w.store.RecordCompanyEmployeeSyncFailureAtVersion(ctx, bindingID, credentialVersion, queueTicket); err != nil {
		w.logger.Printf("employee apply state reconcile failed: binding_id=%d code=SYNC_STATE_FAILED", bindingID)
	}
}

func (w *EmployeeApplyWorker) recordRequeuedState(ctx context.Context, bindingID int, credentialVersion uint64, queueTicket string) {
	if credentialVersion == 0 {
		return
	}
	if err := w.store.MarkCompanyEmployeeSyncQueuedAtVersion(ctx, bindingID, credentialVersion, queueTicket, "SYNC_FAILED"); err != nil {
		w.logger.Printf("employee apply state reconcile failed: binding_id=%d code=SYNC_STATE_FAILED", bindingID)
	}
}
...
func (w *EmployeeApplyWorker) Process(ctx context.Context, event EmployeeApplyEvent) error {
	...
	if err := syncCompanyEmployeesForBinding(ctx, w.store, w.client, int(event.BindingID), event.QueueTicket); err != nil {
