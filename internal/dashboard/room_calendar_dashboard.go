...
func (h *RoomCalendarHandler) Index(w http.ResponseWriter, r *http.Request) {
	...
	_, _, _, _, ok := h.resolveAuthorized(w, r, "/dashboard/roomCalendar/index#get")
	if !ok {
		return
	}
...
func (h *RoomCalendarHandler) Store(w http.ResponseWriter, r *http.Request) {
	...
	userID, user, _, _, ok := h.resolveAuthorized(w, r, "/dashboard/roomCalendar/store#post")
	if !ok {
		return
	}
	corpID, ok := principalCorpID(w, r)
...
func (h *RoomCalendarHandler) Destroy(w http.ResponseWriter, r *http.Request) {
	...
	_, user, _, _, ok := h.resolveAuthorized(w, r, "/dashboard/roomCalendar/destroy#delete")
	if !ok {
		return
	}
	corpID, ok := principalCorpID(w, r)
...
func (h *RoomCalendarHandler) Show(w http.ResponseWriter, r *http.Request) {
	...
	_, _, _, _, ok := h.resolveAuthorized(w, r, "/dashboard/roomCalendar/show#get")
	if !ok {
		return
	}
	corpID, ok := principalCorpID(w, r)
...
func (h *RoomCalendarHandler) resolveAuthorized(w http.ResponseWriter, r *http.Request, permissionKey string) (int, User, DashboardRequestScope, AccessContext, bool) {
...
func (h *RoomCalendarHandler) resolveAccess(w http.ResponseWriter, r *http.Request) (int, User, DashboardRequestScope, bool) {
	if h.resolver == nil {
