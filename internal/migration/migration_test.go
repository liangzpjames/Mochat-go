...
func TestDefaultMigrationsRejectsHistoricalSCRMLeadParityChecksumWhenSQLChanges(t *testing.T) {
	...
	current := []byte("ALTER TABLE mochat_go_scrm_leads ADD COLUMN corp_id bigint;\n")
	if err := os.WriteFile(filepath.Join(migrationPath, "0106_scrm_lead_parity.up.sql"), current, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(migrationPath, "0106_scrm_lead_parity.down.sql"), []byte("ALTER TABLE mochat_go_scrm_leads DROP COLUMN corp_id;\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestStandaloneComposeFreshInitUsesSchemaForCorpDataIndexes(t *testing.T) {
	...
	if latest.Version != "0166_wecom_integration_and_archive_media" {
		t.Fatalf("latest migration = %q, want 0166_wecom_integration_and_archive_media", latest.Version)
	}
