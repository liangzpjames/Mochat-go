...
func assertCallbackRecovery0176Schema(t *testing.T, db *sql.DB, present bool) {
	t.Helper()
	var columns, commandColumns, commands, resources, auditRequestIDLength int
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='mochat_go_wework_callback_side_effect_commands' AND column_name IN ('reservation_token','remaining_unknown_actions')`).Scan(&commandColumns); err != nil {
		t.Fatal(err)
	}
	...
	if present {
		if columns != 6 || commands != 1 || commandColumns != 2 || resources != 3 || auditRequestIDLength != 128 {
			t.Fatalf("0176 schema columns=%d commandColumns=%d commands=%d resources=%d auditRequestIDLength=%d", columns, commandColumns, commands, resources, auditRequestIDLength)
		}
		return
	}
	if columns != 0 || commandColumns != 0 || commands != 0 || resources != 0 || auditRequestIDLength != 96 {
		t.Fatalf("0176 rollback columns=%d commandColumns=%d commands=%d resources=%d auditRequestIDLength=%d", columns, commandColumns, commands, resources, auditRequestIDLength)
	}
	var sideEffects int
