...
func (r *SQLSettingsRepository) List(ctx context.Context, t, c int64, typ string) ([]SCRMSetting, error) {
	q := `SELECT id,tenant_id,corp_id,setting_type,setting_key,label,value_json,enabled,version FROM mochat_go_scrm_settings WHERE tenant_id=? AND corp_id=? AND deleted_at IS NULL`
	a := []any{t, c}
	if typ != "" {
		q += " AND setting_type=?"
		a = append(a, typ)
	...
	for rows.Next() {
		...
		var raw []byte
		var en int
		if e := rows.Scan(&s.ID, &s.TenantID, &s.CorpID, &s.Type, &s.Key, &s.Label, &raw, &en, &s.Version); e != nil {
			return nil, e
		}
		s.Enabled = en != 0
