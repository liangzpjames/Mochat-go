...
func TestPublicPoolHandlerParsesCombinedFiltersAndAuthorizesView(t *testing.T) {
	...
	handler := NewCustomerLifecycleHandler(service, fakePrincipalResolver{principal: Principal{UserID: 42, TenantID: 7, CorpID: 9}}, authorizer)
...
func TestPublicPoolHandlerRequiresClaimUserToMatchPrincipalAndPassesCompleteCommand(t *testing.T) {
	...
	handler := NewCustomerLifecycleHandler(service, fakePrincipalResolver{principal: Principal{UserID: 42, TenantID: 7, CorpID: 9}}, &contactAuthorizerFake{})
	res := httptest.NewRecorder()
